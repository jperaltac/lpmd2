...
void Minimizer::UpdateGlobalMinimum(const ObjectiveFunction& obj) {
  ...
  for (int i = 0; i < nparticles; ++i) {
    // std::cout << "DEBUG rank=" << rank << ": x[" << i << "]=" << x[i] << " with obj " <<
    // obj(x[i]) << "\n";
    if (obj(x[i]) < obj(localmin[i]))
      localmin[i] = x[i];
  }
  State globalmin_proc(localmin[0]);
  for (int i = 0; i < nparticles; ++i) {
    if (obj(localmin[i]) < obj(globalmin_proc))
      globalmin_proc = localmin[i];
  }
  State globalmin(globalmin_proc);
  // Gather all globalmin_proc to rank 0
  for (int i = 0; i < nparams; ++i)
    minibuffer[i] = globalmin_proc[i];
  minibuffer[nparams] = obj(globalmin_proc);
  double globalmin_value = buffer[nparams];
  int i = 0, globalmin_pos = 0;
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Gather(minibuffer, nparams + 1, MPI_DOUBLE, buffer, nparams + 1, MPI_DOUBLE, 0,
             MPI_COMM_WORLD);
...
const State& Minimizer::Minimize(const ObjectiveFunction& obj, const State& seed,
                                 double tolerance) {
  nparams = seed.Size();
  int n = 0;
