...
inline double b(int i, int N, double t) {
  return double(factorial(N)) / double(factorial(N - i) * factorial(i)) * pow(t, i) *
         pow(1.0 - t, N - i);
}

inline double dotb(int i, int N, double t) {
  return double(factorial(N)) / double(factorial(N - i) * factorial(i)) *
         (i * pow(t, i - 1) * pow(1.0 - t, N - i) - pow(t, i) * (N - i) * pow(1.0 - t, N - i - 1));
}

inline double ddotb(int i, int N, double t) {
  return double(factorial(N)) / double(factorial(N - i) * factorial(i)) *
         (i * (i - 1) * pow(t, i - 2) * pow(1.0 - t, N - i) -
          (N - i) * i * pow(t, i - 1) * pow(1.0 - t, N - i - 1) -
          i * pow(t, i - 1) * (N - i) * pow(1.0 - t, N - i - 1) +
          pow(t, i) * (N - i) * (N - i - 1) * pow(1.0 - t, N - i - 2));
}
