...
public:
  ...
  PlusMinus(const Block<double>& block) {
    double S = 0.0, S2 = 0.0;
    for (long int i = 0; i < block.Size(); ++i) {
      S += block[i];
      S2 += (block[i] * block[i]);
    }
    mean = S / double(block.Size());
    stdev = (sqrt(block.Size()) / sqrt(block.Size() - 1)) *
            sqrt(S2 / double(block.Size()) - (mean * mean));
  }
};
...
class WindowAverage {
public:
  WindowAverage(long int nwindow) : n(nwindow) { Clear(); }

  WindowAverage() : n(1) { Clear(); }
...
class SimpleAverage {
public:
  SimpleAverage(long int nwindow) : n(nwindow) { Clear(); }

  SimpleAverage() : n(1) { Clear(); }
