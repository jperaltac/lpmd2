...
public:
  InverseOmega(const RealFunction<T>& h, double emin, double emax, int bins)
      : H(h), emin(emin), emax(emax), bins(bins) {
...
public:
  WangLandau(const RealFunction<T>& energy, double emin, double emax, int bins)
      : energy(energy), emin(emin), emax(emax), bins(bins) {
  ...
  double LogOmega(double E) const { return logp->LogOmega(E); }
