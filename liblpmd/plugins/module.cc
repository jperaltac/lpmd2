...
Module::Module(const Module& mod) {
  ...
  for (long int i = 0; i < kwd.Size(); ++i)
    AssignParameter(kwd[i], mod[kwd[i]]);
}
...
std::string Module::GetNextWord() {
  if (impl->words.Size() == 0)
    throw SyntaxError("Unexpected end of input while reading module arguments");
...
std::string Module::GetNextWord(char d) {
  ...
  if (nextword[0] == d && (nextword[nextword.size() - 1] == e && nextword.size() > 1))
    return nextword;
...
void Module::Show(std::ostream& os) const {
  os << "Module " << impl->name;
  if (Defined("module") && ((*this)["module"] != Name()))
    os << " (loaded as \"" << (*this)["module"] << "\")";
  os << ": " << '\n';
  ...
  if (impl->strictkw) {
    ...
    for (int q = 0; q < tmp.Size(); ++q)
      kwds.Append(Parameter(tmp[q]));
  } else
    kwds = Parameters();
  for (long int i = 0; i < kwds.Size(); ++i) {
    if ((kwds[i]) == "module")
      continue; // No muestra parametro "module", reservado para uso interno
    if ((kwds[i]) == "fullpath")
      continue; // parametro reservado
    if (impl->emptycall) {
      os << "   " << std::setw(20) << (kwds[i]);
      if (impl->defvalues.Defined(kwds[i]))
        os << " = " << std::setw(30) << impl->defvalues[kwds[i]] << '\n';
      else
        os << " has no default value\n";
    } else {
      os << "   " << std::setw(20) << (kwds[i]) << " = " << std::setw(30) << (*this)[kwds[i]];
      if (impl->defvalues.Defined(kwds[i]))
        os << " (default: " << impl->defvalues[kwds[i]] << ")\n";
      else
        os << '\n';
    }
  }
...
void Module::SetDebugStream(std::ostream& ostr) {
  impl->extdebugstr = false;
  impl->debugstr = &ostr;
}
...
double Module::GetProperty(const std::string& name) {
  std::cerr << "[Warning] The module \"" << name
            << "\" does not implement access to any properties." << '\n';
  return 0.0;
}
...
PluginManager& Module::GetManager() const {
  if (impl->manager == NULL)
    throw PluginError(impl->name, "Called GetManager() on an unmanaged plugin");
  return *(impl->manager);
}
