...
struct PluginDeleter
{
 void operator()(Plugin * plugin) const noexcept
 {
  ...
  if (destroy != NULL) destroy(plugin);
  else delete plugin;
...
PluginManager::PluginManager()
{
 ...
 {
  for (const auto & entry : SplitPathList(pathvariable)) AddToPluginPath(entry);
 }
 for (const auto & entry : SplitPathList(PLUGINPATH)) AddToPluginPath(entry);
}

PluginManager::~PluginManager()
{
 Array<std::string> kill_list;
 for (auto it=modules.begin();it != modules.end();++it) kill_list.Append(it->first);
...
Plugin & PluginManager::operator[](std::string id)
{
 ...
 else return *(modules[id].get()); 
}

const Plugin & PluginManager::operator[](std::string id) const
{
 if (modules.count(id) == 0) throw InvalidRequest("Plugin "+id);
 const std::map<std::string, std::unique_ptr<Plugin, PluginDeleter>>::const_iterator & p = modules.find(id);
 return *((*p).second);
}
