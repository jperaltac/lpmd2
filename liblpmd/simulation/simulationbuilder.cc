...
template <typename AtomContainer = lpmd::ParticleSet, typename CellType = lpmd::Cell>
class SimulationEngine : public Simulation {
public:
  explicit SimulationEngine(RuntimeContext& context)
      : atoms(0), cell(0), integ(0), velocitiesSet(false), initialized(false), step(0) {
  ...
  SimulationEngine(RuntimeContext& context, long int natoms)
      : atoms(0), cell(0), integ(0), velocitiesSet(false), initialized(false), step(0) {
    SetRuntimeContext(context);
  ...
  SimulationEngine(RuntimeContext& context, long int natoms, const BasicAtom& t)
      : atoms(0), cell(0), integ(0), velocitiesSet(false), initialized(false), step(0) {
    SetRuntimeContext(context);
  ...
  void SetTemperature(double temp, bool tag) {
    if (!velocitiesSet)
      InitVelocities();
    BasicParticleSet& atomaccesor =
        Atoms(); // en vez de (*atoms), para que funcione con ApplyAtomMask
    double ti = Temperature(atomaccesor, tag);

    if (tag == false) {
      for (long int i = 0; i < atomaccesor.Size(); ++i)
        atomaccesor[i].Velocity() *= sqrt(temp / ti);
    } else {
      for (long int i = 0; i < atomaccesor.Size(); ++i) {
        if (atomaccesor.Have(atomaccesor[i], Tag("fixedvel")) ||
            atomaccesor.Have(atomaccesor[i], Tag("fixedpos")))
          continue;
        else
          atomaccesor[i].Velocity() *= sqrt(temp / ti);
      }
  ...
  void SetSimulationTags() {
    ...
    const Vector P =
        Momentum(Atoms(), (Atoms().HaveAny(Tag("fixedvel")) || Atoms().HaveAny(Tag("fixedpos"))));
    SetTag(*this, Tag("step"), step);
    SetTag(*this, Tag("temperature"),
           Temperature(Atoms(),
                       (Atoms().HaveAny(Tag("fixedvel")) || Atoms().HaveAny(Tag("fixedpos")))));
    ...
    double kinenerg = KineticEnergy(
        Atoms(), (Atoms().HaveAny(Tag("fixedvel")) || Atoms().HaveAny(Tag("fixedpos"))));
    ...
    SetTag(*this, Tag("pressure"),
           (pressfactor / v) * ((2.0 / 3.0) * kinenerg + (1.0 / 3.0) * Virial()));
  ...
  lpmd::Integrator& Integrator() {
    if (integ == 0)
      throw MissingComponent("integrator");
    return (*integ);
  }
  ...
  void RescalePositions(const BasicCell& old_cell) {
    ...
    for (long int i = 0; i < t_atoms.Size(); ++i)
      t_atoms[i].Position() =
          t_cell.FittedInside(t_cell.Cartesian(old_cell.Fractional(t_atoms[i].Position())));
  }
...
SimulationBuilder::~SimulationBuilder() {
  for (long int i = 0; i < impl.s_array.Size(); ++i)
    delete impl.s_array[i];
}

Simulation& SimulationBuilder::CreateFixedOrthogonal(RuntimeContext& context, long int atoms,
                                                     const BasicAtom& at) {
...
Simulation& SimulationBuilder::CreateGeneric(RuntimeContext& context, long int atoms,
                                             const BasicAtom& at) {
...
Simulation& SimulationBuilder::CloneOptimized(RuntimeContext& context, const Simulation& sim) {
  ...
  if (cell.IsOrthogonal()) {
    simp =
        new SimulationEngine<FixedSizeParticleSet, OrthogonalCell>(context, atoms.Size(), atoms[0]);
  } else {
    simp = new SimulationEngine<FixedSizeParticleSet, NonOrthogonalCell>(context, atoms.Size(),
                                                                         atoms[0]);
  }
  ...
  for (long int i = 0; i < atoms.Size(); ++i) {
    ...
    // if (ColorHandler::HaveColor(basic_i)) ColorHandler::ColorOfAtom(real_i) =
    // ColorHandler::ColorOfAtom(basic_i);
