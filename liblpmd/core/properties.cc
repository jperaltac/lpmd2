...
void lpmd::gdr(lpmd::Configuration& con, lpmd::Potential& pot, long int nb, double rcut,
               lpmd::Matrix& m) {
  ...
  for (int i = 0; i < pairs.Size(); ++i) {
    ...
    int ne1 = 0, ne2 = 0;
    for (int a = 0; a < N; a++) {
      if (atoms[a].Symbol() == loa[0])
        ne1++;
      if (atoms[a].Symbol() == loa[1])
        ne2++;
    }
  ...
  for (long int i = 0; i < pairs.Size(); ++i) {
    ...
    for (int k = 0; k < nb; k++) {
      if (e1 == e2)
        gt[k] = gt[k] + ce1 * ce2 * g[k][j];
      else {
        gt[k] = gt[k] + 2 * ce1 * ce2 * g[k][j];
      }
    }
    j++;
  }
...
void lpmd::vacf(lpmd::ConfigurationSet& hist, lpmd::Potential& pot, double dt, lpmd::Matrix& m) {
  ...
  if (level == 0) {
    ...
    //
    for (int i = 0; i < nat; ++i)
      velocities[0][i] = (noperiodic[0][i] - noperiodic[1][i]) / dt;
  ...
  for (int e1 = 0; e1 < nsp; e1++) {
    int ne = 0;
    for (int i = 0; i < nat; i++) {
      if (hist[0].Atoms()[i].Z() == species[e1])
        ne++;
    }
