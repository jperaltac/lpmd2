...
Matrix::Matrix(long cols, long rows)
    : nr(rows), nc(cols), values(static_cast<std::size_t>(rows * cols), 0.0) {
  col_labels.Clear();
...
Matrix::Matrix(const Matrix& m)
    : nr(m.Rows()), nc(m.Columns()), values(m.values), col_labels(m.col_labels) {}
...
double Matrix::Det() const {
  if (nr != nc)
    throw InvalidOperation("Determinant of non-square matrix");
  if (nr == 0)
    return 0.0;
  ...
  for (long i = 0; i < n; ++i) {
    ...
    if (pivot_value <= std::numeric_limits<double>::epsilon())
      return 0.0;
    if (pivot_row != i) {
      for (long c = 0; c < n; ++c) {
...
void Matrix::Inverse() {
  if (nc != nr)
    throw InvalidOperation("Inverse of non-square matrix");
  const long n = nr;
  if (n == 0)
    throw InvalidOperation("Inverse of empty matrix");

  std::vector<double> a = values;
  std::vector<double> inverse(static_cast<std::size_t>(n * n), 0.0);
  for (long i = 0; i < n; ++i)
    inverse[Index(n, i, i)] = 1.0;
  ...
  for (long i = 0; i < n; ++i) {
    ...
    if (pivot_value <= eps)
      throw InvalidOperation("Matrix is singular and cannot be inverted");
    if (pivot_row != i) {
      for (long c = 0; c < n; ++c) {
        std::swap(a[Index(n, c, i)], a[Index(n, c, pivot_row)]);
...
Matrix& Matrix::operator+=(const Matrix& m) {
  if ((Rows() == 0) && (Columns() == 0))
    ...
    throw InvalidOperation("Adding matrices with different size");
}
...
Matrix lpmd::operator+(const Matrix& a, const Matrix& b) {
  if ((a.Rows() == b.Rows()) && (a.Columns() == b.Columns())) {
    ...
    for (long j = 0; j < a.Rows(); ++j)
      for (long i = 0; i < a.Columns(); ++i)
        s.Set(i, j, s.Get(i, j) + b.Get(i, j));
    return s;
  } else
    throw InvalidOperation("Adding matrices with different size");
}
