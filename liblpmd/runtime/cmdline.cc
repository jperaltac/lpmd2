...
void CommandArguments::DefineOption(const std::string& longname, const std::string& shortname,
...
void CommandArguments::Parse(int argc, const char** argv) {
  ...
  for (int i = 0; i < argc; ++i) {
    if (!strcmp(argv[i], "-"))
      impl.arglist.Append(std::string(argv[i]));
      ...
      if (optfound == false)
        throw SyntaxError("Unknown command line option, --" + optname);
