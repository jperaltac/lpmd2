...
void ControlFile::Read(const std::string& inpfile, const ParamList& options) {
  if (inpfile == "-")
    Read(std::cin, options, "-");
  else {
    std::ifstream LoadSystem;
    LoadSystem.open(inpfile.c_str(), std::ios::in);
    if (!LoadSystem.good())
      throw FileNotFound(inpfile);
    Read(LoadSystem, options, inpfile);
  }
}

inline std::string AddBlockContent(const std::string& blockname, const std::string& terminator,
                                   std::istream& istr, Array<std::string>& tmpwords) {
  std::string inpbuffer = (blockname + "block ");
  for (int z = 1; z < tmpwords.Size(); ++z)
    inpbuffer += (tmpwords[z] + " ");
  while (1) {
    std::string line;
    if (istr.eof())
      throw SyntaxError("\"" + blockname + "\" block was not properly closed with \"" + terminator +
                        "\"");
    getline(istr, line);
...
void ControlFile::ReadLine(const std::string& line) {
  ...
  if (tmp.find("$(", 0) != std::string::npos)
    throw SyntaxError("Variable(s) undefined in input file \"" + impl->filename + "\", line " +
                      ToString(impl->line_count) + "\n  " + tmp);
  //
  ...
  for (int q = 0; q < impl->blockheaders.Size(); ++q) {
    if (first_word == (impl->blockheaders[q] + "block")) {
      blockpos = q;
      break;
    }
  }
  if (blockpos != -1) {
    if (OnBlock(impl->blockheaders[blockpos], tmp) != 0)
      throw SyntaxError("Invalid content of \"" + impl->blockheaders[blockpos] +
                        "\" block, in input file \"" + impl->filename + "\", line " +
                        ToString(impl->line_count) + "\n  " + tmp);
  } else {
    if (statement_args != "") {
      ...
      if (OnRegularStatement(first_word, st_keywords) != 0)
        throw SyntaxError("Unexpected error in input file \"" + impl->filename + "\", line " +
                          ToString(impl->line_count) + "\n  " + tmp);
    } else {
      if (OnNonRegularStatement(first_word, tmp) != 0)
        throw SyntaxError("Unexpected instruction was found in input file \"" + impl->filename +
                          "\", line " + ToString(impl->line_count) + "\n  " + tmp);
    }
  }
}
...
void ControlFile::Read(std::istream& real_istr, const ParamList& options,
  ...
  while (!real_istr.eof()) {
    ...
    for (int q = 0; q < impl->blockheaders.Size(); ++q) {
      if (tmpwords[0] == impl->blockheaders[q]) {
        inpbuffer +=
            AddBlockContent(impl->blockheaders[q], impl->blockfooters[q], real_istr, tmpwords);
        foundblock = true;
        break;
      }
    }
...
std::string ControlFile::ParseCommandArguments(Map& param, const std::string& cmd,
  ...
  while (impl->words.Size() > 0) {
    ...
    bool ready = true;
    for (int q = 0; q < kvect.Size(); ++q)
      if (bool(resolved[kvect[q]]) == false)
        ready = false;
    if (ready)
      break;
  }
...
std::string ControlFileImpl::GetNextWord() {
  if (words.Size() == 0)
    throw SyntaxError("reading control file " + filename);
...
std::string ControlFileImpl::MatchCommand() {
  std::string tmp;
  for (std::map<std::string, std::string>::const_iterator it = reservedkw.begin();
       it != reservedkw.end(); ++it) {
