...
void ExtractCorrectPairs(Array<Pair>& ap) {
  ...
  for (long int i = 0; i < ap.Size(); ++i) {
    if ((long int)(ap[i].i) < (long int)(ap[i].j))
      tmp.Append(ap[i]);
  }
  ap = tmp;
}

template <typename AtomContainer, typename CellType>
void GenerateFCC(AtomContainer& atoms, CellType& cell) {
  long int cc = 0;
  double ax = 1.0 / 3.0;
  double ay = 1.0 / 3.0;
  double az = 1.0 / 3.0;
  for (long k = 0; k < 3; ++k)
    for (long j = 0; j < 3; ++j)
      for (long i = 0; i < 3; ++i) {
        atoms[cc++].Position() = cell.ScaleByCell(
            Vector((double(i) + 0.5) * ax, (double(j) + 0.5) * ay, (double(k) + 0.5) * az));
        atoms[cc++].Position() = cell.ScaleByCell(
            Vector((double(i) + 0.5) * ax, double(j) * ay, (double(k) + 1.0) * az));
        atoms[cc++].Position() = cell.ScaleByCell(
            Vector((double(i) + 1.0) * ax, double(j) * ay, (double(k) + 0.5) * az));
        atoms[cc++].Position() = cell.ScaleByCell(
            Vector((double(i) + 1.0) * ax, (double(j) + 0.5) * ay, (double(k) + 1.0) * az));
      }
  CenterByCenterOfMass(atoms, cell);
}

template <typename AtomContainer, typename CellType>
void CheckAllInside(AtomContainer& atoms, CellType& cell) {
...
int main() {
  ...
  for (long int i = 0; i < atoms.Size(); ++i) {
    ...
    if (nlist.Size() != 12) {
      std::cerr << "DEBUG Atom " << i << ", wrong number of neighbors: " << nlist.Size()
                << ", should be 12\n";
    } else {
