...
int main() {
  ...
  CellGenerator& cg =
      pm.LoadPluginAs<CellGenerator>("crystal3d", "type fcc symbol Ar nx " + ToString(NX) + " ny " +
                                                      ToString(NY) + " nz " + ToString(NZ));
  cg.Generate(md);
  ...
  Potential& pot =
      pm.LoadPluginAs<Potential>("lennardjones", "sigma 3.41 epsilon 0.0103408 cutoff 8.5");
  ...
  for (long i = 0; i < nsteps; ++i) {
    for (int k = 0; k < atoms.Size(); ++k)
      atoms[k].Acceleration() = Vector(0.0, 0.0, 0.0);
    md.DoStep();
    if (i % 100 == 0) {
      double kin_en = KineticEnergy(atoms), pot_en = 0.0;
      for (int q = 0; q < potentials.Size(); ++q) {
        pot_en += potentials[q].energy(md);
      }
