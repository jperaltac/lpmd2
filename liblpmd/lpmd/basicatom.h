...
class BasicAtom {
public:
  BasicAtom(int z0, Vector* ip, Vector* iv, Vector* ia)
      : z(z0), ipos(ip), ivel(iv), iacc(ia), mass(ElemMass[z]), charge(0.0), id(LastAtomID++) {}

  BasicAtom(int z0, Vector* ip, Vector* iv, Vector* ia, unsigned long int _id)
      : z(z0), ipos(ip), ivel(iv), iacc(ia), mass(ElemMass[z]), charge(0.0), id(_id) {}
