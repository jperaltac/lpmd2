...
public:
  ...
  Box(double xmin, double xmax, double ymin, double ymax, double zmin, double zmax, Vector _a,
      Vector _b, Vector _c) {
    vmin[0] = xmin;
    vmin[1] = ymin;
    vmin[2] = zmin;
  ...
  inline bool IsInside(const Vector& v) const {
    ...
    if ((fabs(vp[0]) < vmin[0]) || (fabs(vp[0]) > vmax[0]))
      return false;
    if ((fabs(vp[1]) < vmin[1]) || (fabs(vp[1]) > vmax[1]))
      return false;
    if ((fabs(vp[2]) < vmin[2]) || (fabs(vp[2]) > vmax[2]))
      return false;
    return true;
  }
...
public:
  Cylinder(const Vector& S, const Vector& origin, double rmax, double rmin)
      : _S(S), _origin(origin), _rmax(rmax), _rmin(rmin) {}
...
public:
  Cone(const Vector& tip, const Vector& bot, double alpha, double beta)
      : _tip(tip), _bot(bot), _alpha(alpha), _beta(beta) {}
  ...
  inline bool IsInside(const Vector& v) const {
    ...
    if (angle >= _alpha || angle < _beta)
      return false;
    if (posrel.Module() > direct.Module())
      return false;
    else
      return true;
  }
