...
public:
  ...
  Array(const T* buffer, long int s) : std::vector<T>(s) {
    for (long int i = 0; i < s; ++i)
      (static_cast<std::vector<T>&>(*this))[i] = buffer[i];
  }

  inline T& operator[](long int i) { return (static_cast<std::vector<T>&>(*this)).operator[](i); }

  inline const T& operator[](long int i) const {
    return (static_cast<const std::vector<T>&>(*this)).operator[](i);
  }
...
public:
  ...
  inline T& operator[](long int i) {
    return *((static_cast<std::vector<T*>&>(*this)).operator[](i));
  }
