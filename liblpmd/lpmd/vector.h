    ...
    if (*p == '<') {
      ...
      while (*p != '\0') {
        if ((*p == ',') || (*p == '>')) {
          ...
          (*p) = '\0';
          if (k == 3)
            throw SyntaxError("Too many commas, GenericVector(" + std::string(str) + ")");
          v[k++] = atof(start);
          if (token == '>')
            break;
          p++;
      ...
      if (k != 3)
        throw SyntaxError("GenericVector(" + std::string(str) + ")");
    } else {
      start = p;
      while (1) {
        if (isblank(*p) || (*p == '\0')) {
          if (k == 3)
            throw SyntaxError("Too many delimiters, GenericVector(" + std::string(str) + ")");
          (*p) = '\0';
          v[k++] = atof(start);
          p++;
          start = p;
