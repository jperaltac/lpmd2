...
public:
  TagHandler<T>& operator=(const TagHandler<T>& orig) {
    if (&orig != this) {
      for (typename std::map<Tag, ParamList*>::const_iterator it = orig.namedcontainers.begin();
           it != orig.namedcontainers.end(); ++it) {
        if (namedcontainers.count(it->first) != 0)
          delete namedcontainers[it->first];
        namedcontainers[it->first] = new ParamList();
        *(namedcontainers[it->first]) = *(it->second);
      }
      for (typename std::map<unsigned long int, ParamList*>::iterator it = orig.objmapper.begin();
           it != orig.objmapper.end(); ++it) {
        if (objmapper.count(it->first) != 0)
          delete objmapper[it->first];
        objmapper[it->first] = new ParamList();
        *(objmapper[it->first]) = *(it->second);
      }
      for (typename std::map<Tag, int>::const_iterator it = orig.tagregistry.begin();
           it != orig.tagregistry.end(); ++it)
        tagregistry[it->first] = 1;
      for (typename std::map<unsigned long int, int>::const_iterator it = orig.inside_named.begin();
           it != orig.inside_named.end(); ++it)
  ...
  ~TagHandler() {
    for (typename std::map<Tag, ParamList*>::const_iterator it = namedcontainers.begin();
         it != namedcontainers.end(); ++it)
      delete (it->second);
    for (typename std::map<unsigned long int, ParamList*>::const_iterator it = objmapper.begin();
         it != objmapper.end(); ++it) {
      if (inside_named.count(it->first) == 0)
        delete (it->second);
    }
  }
  ...
  Map& GetTagGroup(Tag& container_tag) {
    if (namedcontainers.count(container_tag) == 0)
      throw InvalidRequest("Tag " + container_tag);
  ...
  bool Have(const T& obj, Tag& tag) const {
    if (objmapper.count(obj.ID()) == 0)
      return false;
    try {
      const typename std::map<unsigned long int, ParamList*>::const_iterator& p =
          objmapper.find(obj.ID());
  ...
  bool HaveAny(Tag& tag) {
    if (tagregistry.count(tag) > 0)
      return true;
    for (typename std::map<Tag, ParamList*>::const_iterator it = namedcontainers.begin();
         it != namedcontainers.end(); ++it)
      if ((it->second)->Defined(tag))
        return true;
    return false;
  }
  ...
  void RemoveTags(const T& obj) {
    if (objmapper.count(obj.ID()) != 0) {
      ...
      for (typename std::map<Tag, ParamList*>::const_iterator it = namedcontainers.begin();
           it != namedcontainers.end(); ++it) {
        if (it->second == objmapper[obj.ID()]) {
          todelete = it->first;
          break;
        }
      }
      if (todelete != "")
        namedcontainers.erase(todelete);
  ...
  const std::string GetTag(const T& obj, Tag& tag) {
    if (objmapper.count(obj.ID()) == 0)
      throw InvalidRequest("Tag " + tag);
    try {
      return objmapper[obj.ID()]->operator[](tag);
    } catch (std::exception& e) {
      throw InvalidRequest("Tag " + tag);
    }
  }
