...
public:
  ...
  NonOrthogonalCell(const double a, const double b, const double c, const double alpha,
  ...
  NonOrthogonalCell(std::string str) {
    ...
    if (str[0] == '<') {
      ...
      (*this) =
          NonOrthogonalCell(Vector(bv[0].c_str()), Vector(bv[1].c_str()), Vector(bv[2].c_str()));
    } else {
      Array<std::string> vecs = StringSplit(str, ' ');
      if (vecs.Size() == 3) {
        for (int q = 0; q < 3; ++q)
          v[q] = Vector(vecs[q].c_str());
      } else if (vecs.Size() == 6) {
        NonOrthogonalCell tmp(atof(vecs[0].c_str()), atof(vecs[1].c_str()), atof(vecs[2].c_str()),
                              atof(vecs[3].c_str()), atof(vecs[4].c_str()), atof(vecs[5].c_str()));
        (*this) = tmp;
      } else if (vecs.Size() == 9) {
        for (int p = 0; p < 3; ++p)
          for (int q = 0; q < 3; ++q)
            v[p][q] = atof(vecs[q + 3 * p].c_str());
      } else {
        // FIXME : Error!
      }
    }
  ...
  bool IsInside(const Vector& a) const {
    ...
    ConvertToInternal(vtmp);
    for (int q = 0; q < 3; ++q)
      if ((vtmp[q] < 0.0) || (vtmp[q] > v[q].Module()))
        return false;
    return true;
  }
...
inline lpmd::NonOrthogonalCell::NonOrthogonalCell(const Vector& a, const Vector& b,
                                                  const Vector& c) {
  v[0] = a;
  v[1] = b;
  v[2] = c;
...
inline lpmd::NonOrthogonalCell::NonOrthogonalCell(const double a, const double b, const double c,
