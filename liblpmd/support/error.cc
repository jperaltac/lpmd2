...
NotImplemented::NotImplemented(const std::string functionality)
    : Error(functionality + " is not implemented.") {}

InvalidOperation::InvalidOperation(const std::string operation)
...
PluginError::PluginError(const std::string pluginname, const std::string description)
    : RuntimeError("(in plugin " + pluginname + "): " + description) {}

InvalidRequest::InvalidRequest(const std::string request) : RuntimeError(request + " not found.") {}

MissingComponent::MissingComponent(const std::string component)
    : RuntimeError(component + " is needed for the process.") {}
...
FileNotFound::FileNotFound(const std::string filename)
    : SystemError("File " + filename + " not found.") {}
