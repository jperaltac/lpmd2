...
bool Mol2Format::ReadCell(std::istream& is, Configuration& con) const {
  ...
  if ((*this)["replacecell"] == "true")
    throw PluginError("mol2", "This format does not contain any cell vectors.");
  long natoms = 0;
  if (is.eof())
    return false;
  while (!is.eof()) {
    ...
    if (tmp[0] != '#') {
      ...
      if (tmp == "@<TRIPOS>ATOM") {
        if (natoms <= 0)
          throw PluginError("mol2", "Atoms number in mol2 file was not be read!");
        else {
          for (int i = 0; i < natoms; i++) {
            getline(is, tmp);
            Array<std::string> linea = StringSplit(tmp, ' ');
            std::string symb = linea[1];
            lpmd::Vector pos(atof(linea[2].c_str()), atof(linea[3].c_str()),
                             atof(linea[4].c_str()));
            if (symb.length() > 1) {
