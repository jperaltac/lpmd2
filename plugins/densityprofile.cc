...
void DensityProfile::SetParameter(std::string name) {
  ...
  if (name == "axis") {
    ...
    if ((eje == "X" || eje == "x") || eje == "0")
      axis = 0;
    else if ((eje == "Y" || eje == "y") || eje == "1")
      axis = 1;
    else if ((eje == "Z" || eje == "z") || eje == "2")
      axis = 2;
    else {
      axis = -1;
    ...
    if (range[tmp][0] >= range[tmp][1] && smin != "all" && smin != "ALL")
      lpmd::ShowWarning("densityprofile", "min and max values are not consistent.");
  } else
    Module::SetParameter(name);
}
...
void DensityProfile::ShowHelp() const {
  ...
  std::cout
      << "      axis          : Sets the axis in which the evaluation will be made (X/Y/Z).\n";
...
void DensityProfile::Evaluate(lpmd::Configuration& con, lpmd::Potential& pot) {
  ...
  if (bins == 0)
    throw lpmd::PluginError("densityprofile", "Error in calculation: Wrong value for \"bins\".");

  // Vectores base, celda de simulacion.
  if (fabs(range[0][0]) < 1E-5 && fabs(range[0][1]) < 1E-5) {
    range[0][0] = 0;
    range[0][1] = (cell[0]).Module();
  }
  if (fabs(range[1][0]) < 1E-5 && fabs(range[1][1]) < 1E-5) {
    range[1][0] = 0;
    range[1][1] = (cell[1]).Module();
  }
  if (fabs(range[2][0]) < 1E-5 && fabs(range[2][1]) < 1E-5) {
    range[2][0] = 0;
    range[2][1] = (cell[2]).Module();
  }

  if (fabs(range[0][0] - range[0][1]) < 1E-5)
    throw lpmd::PluginError("densityprofile", "Error in cell range in axis X.");
  if (fabs(range[1][0] - range[1][1]) < 1E-5)
    throw lpmd::PluginError("densityprofile", "Error in cell range in axis Y.");
  if (fabs(range[2][0] - range[2][1]) < 1E-5)
    throw lpmd::PluginError("densityprofile", "Error in cell range in axis Z.");

  lpmd::Vector na = cell[0];
  na.Normalize();
  lpmd::Vector nb = cell[1];
  nb.Normalize();
  lpmd::Vector nc = cell[2];
  nc.Normalize();
  ...
  if (axis == 0) {
    dr = la.Module() / double(bins);
  } else if (axis == 1) {
    dr = lb.Module() / double(bins);
  } else if (axis == 2) {
    dr = lc.Module() / double(bins);
  } else {
    throw lpmd::PluginError("densityprofile", "Error in axis setting to set 'dr'!.");
  }
          ...
          if (y >= range[1][0] && y <= range[1][1]) {
            if (z >= range[2][0] && z <= range[2][1]) {
              ...
              if (axis == 0)
                ...
                lpmd::ShowWarning("plugin densityprofile",
                                  "Bad calculation of densityprofile, check your 'axis' option.");
