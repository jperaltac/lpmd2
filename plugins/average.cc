...
AverageVisualizer::AverageVisualizer(std::string args) : Plugin("average", "1.0") {
  ...
  if (params["output"] != "-")
    output_stream = new std::ofstream(params["output"].c_str());
  else
  ...
  for (int i = 0; i < property_array.Size(); ++i)
    property_averages[i] = last_value[i] = 0.0;
  average_count = 0;
}
...
void AverageVisualizer::ShowHelp() const {
  ...
  std::cout
      << "      interval      : Sets the interval in which the properties will be averaged.\n";
  ...
  std::cout << " average properties=temperature,sxx interval=100 start=0 end=-1 each=50 "
               "output=prop.dat\n";
  ...
  std::cout
      << "      file prop.dat during all the simulation (end=-1), each 50 simulation steps.\n";
