...
bool XYZFormat::ReadCell(std::istream& is, Configuration& sc) const {
  if ((*this)["replacecell"] == "true")
    throw PluginError("xyz", "This format does not contain any cell vectors.");
  std::string tmp;
  ...
  delete[] bfold; // hay que hacer delete sobre el antiguo valor, no sobre el nuevo ya que strtol lo
                  // cambia
  ...
  for (long i = 0; i < natoms; ++i) {
    ...
    if (words.Size() == 4) {
      ...
      throw PluginError("xyz", "An unidentified line was found in the file \"" + readfile +
  ...
  if (zerocm == "true") {
    ...
    for (long i = 0; i < natoms; ++i)
      part[i].Position() += (geocenter - (cm / totalmass));
  }
  Vector cntrd(0.5, 0.5, 0.5);
  Vector cntrdXY(0.5, 0.5, 0.0);
  if (coords == "centered")
    UnCenter(part, cell, cntrd);
  if (coords == "centeredXY")
    UnCenter(part, cell, cntrdXY);
  if (external != "ignore") {
      ...
      for (int i = 0; i < natoms; i++) {
        if (!cell.IsInside(part[i].Position())) {
          if (external == "delete") {
            ...
            throw PluginError("xyz",
                              "The atom[" + ToString<int>(i) + "] was found outside the cell.");
          }
        }
      }
      if (external == "delete")
        DebugStream() << "-> Atoms deleted ouside the cell : " << dc << '\n';
    }
  }
  return true;
}
...
void XYZFormat::WriteCell(std::ostream& out, Configuration& sc) const {
  ...
  if (level == 0) {
    for (long int i = 0; i < part.Size(); i++)
      out << part[i].Symbol() << std::fixed << std::setprecision(precout) << " "
          << part[i].Position() << std::endl;
  } else if (level == 1) {
    for (long int i = 0; i < part.Size(); i++)
      out << part[i].Symbol() << std::fixed << std::setprecision(precout) << " "
          << part[i].Position() << " " << part[i].Velocity() << std::endl;
  } else if (level == 2) {
    for (long int i = 0; i < part.Size(); i++)
      out << part[i].Symbol() << std::fixed << std::setprecision(precout) << " "
          << part[i].Position() << " " << part[i].Velocity() << " " << part[i].Acceleration()
          << std::endl;
  }
}
