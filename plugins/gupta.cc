...
Vector Gupta::ManyBodies(const Vector& normrij, const double& rhoi, const double& rhoj,
                         const double& rmod) const {
  double tmp = (B * q / r0) * ((1.0e0 / sqrt(rhoi)) + (1.0e0 / sqrt(rhoj))) *
               exp(-2.0e0 * q * (rmod - r0) / r0);
  return tmp * normrij;
}
