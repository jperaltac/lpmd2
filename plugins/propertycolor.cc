...
void PropertyColorModifier::Apply(Simulation& sim) {
  ...
  for (long int i = 0; i < atoms.Size(); ++i) {
    ...
    if (property == "temperature")
      v = (1.0 / 3.0) * (kin2ev / kboltzmann) * atoms[i].Mass() *
          atoms[i].Velocity().SquareModule();
    else if (property == "velocity")
      v = atoms[i].Velocity().SquareModule();
    else if (property == "acceleration")
      v = atoms[i].Acceleration().SquareModule();
      ...
      v = vmin + (vmax - vmin) * drand48();
    else if (property == "external") {
      getline(*extfile, line);
      Array<std::string> lspl = StringSplit(line);
      v = strtod(lspl[column - 1].c_str(), 0);
    } else
      throw PluginError("propertycolor", "Cannot color atoms by property \"" + property + "\"");
