  ...
  for (long int i = 0; i < atoms.Size(); ++i) {
    ...
    for (int j = 0; j < tags.Size(); ++j) {
      ...
      if (tag == "x")
        ...
        std::cout << acc[2];
      else if ((tag == "r") && ColorHandler::HaveColor(atoms[i]))
        std::cout << ColorHandler::ColorOfAtom(atoms[i])[0];
      else if ((tag == "g") && ColorHandler::HaveColor(atoms[i]))
        std::cout << ColorHandler::ColorOfAtom(atoms[i])[1];
      else if ((tag == "b") && ColorHandler::HaveColor(atoms[i]))
        std::cout << ColorHandler::ColorOfAtom(atoms[i])[2];
      else if (atoms.Have(atoms[i], Tag(tag)))
        std::cout << atoms.GetTag(atoms[i], Tag(tag));
      std::cout << " ";
    }
    std::cout << '\n';
  }
