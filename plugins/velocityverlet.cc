...
void VelocityVerlet::Initialize(Simulation& sim, Potential& p) {
  assert(&p != 0);
  UseOldConfig(sim);
}
...
void VelocityVerlet::AdvancePosition(Simulation& sim, long i) {
  ...
  Vector newpos =
      atoms[i].Position() + atoms[i].Velocity() * dt + 0.5 * atoms[i].Acceleration() * dt * dt;
...
void VelocityVerlet::AdvanceVelocity(Simulation& sim, long i) {
  ...
  lpmd::BasicParticleSet& oldatoms = OldConfig().Atoms();
  atoms[i].Velocity() =
      atoms[i].Velocity() + 0.5 * dt * (oldatoms[i].Acceleration() + atoms[i].Acceleration());
}
