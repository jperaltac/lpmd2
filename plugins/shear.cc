...
ShearModifier::ShearModifier(std::string args) : Plugin("shear", "2.1") {
  ...
  if ((param["axis"] == "x") || (param["axis"] == "X"))
    shear_axis = 0;
  else if ((param["axis"] == "y") || (param["axis"] == "Y"))
    shear_axis = 1;
  else if ((param["axis"] == "z") || (param["axis"] == "Z"))
    shear_axis = 2;
  else
    throw PluginError("shear", "Invalid shear axis");
  if ((param["normal"] == "x") || (param["normal"] == "X"))
    perp_axis = 0;
  else if ((param["normal"] == "y") || (param["normal"] == "Y"))
    perp_axis = 1;
  else if ((param["normal"] == "z") || (param["normal"] == "Z"))
    perp_axis = 2;
  else
    throw PluginError("shear", "Invalid normal axis");
  if (shear_axis == perp_axis)
    throw PluginError("shear", "Shear axis must be orthogonal to normal axis");
