...
double NullMetalPotential::pairEnergy(const double& r) const {
  assert(&r != 0);
  return 0.0;
} // icc 869

double NullMetalPotential::rhoij(const double& r) const {
  assert(&r != 0);
  return 0.0;
} // icc 869

double NullMetalPotential::F(const double& rhoi) const {
  assert(&rhoi != 0);
  return 0.0;
} // icc 869
...
Vector NullMetalPotential::ManyBodies(const Vector& normrij, const double& invrhoi,
                                      const double& invrhoj, const double& mod) const {
  assert(&normrij != 0); // icc 869
...
double NullMetalPotential::deltarhoi(const double& rhobar) const {
  assert(&rhobar != 0);
  return 0.0;
} // icc 869
...
double NullMetalPotential::VirialCorrection(const double& rhobar, const int& N,
                                            const double& rhoi) const {
  assert(&rhobar != 0); // icc869
  assert(&N != 0);      // icc 869
  assert(&rhoi != 0);   // icc 869
  return 0.0;
...
Vector NullMetalPotential::UpdateCorrections(const double& rho, const int& N,
                                             const double& sinv) const {
