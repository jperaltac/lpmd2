...
bool VaspFormat::ReadCell(std::istream& is, Configuration& con) const {
  ...
  double x, y, z;
  if (first)
    getline(is, tmp); // Read title/atoms for POSCAR/XDATCAR.
  if (is.eof())
    return false; // no more configurations to read
  ...
  if (first) {
    ...
    if (ftype == "poscar" || ftype == "contcar") {
      ...
      for (int i = 0; i < thisline.Size(); ++i) {
        ...
        if (i > 0 && isnumber != isnumber0)
          throw PluginError("vasp",
                            "Error, mixed type of variables on line 6 of POSCAR/CONTCAR file.");
        isnumber0 = isnumber;
      }
      ...
      if (!isnumber) // VASP 5.2
      {
        for (int i = 0; i < thisline.Size(); ++i) // Here, 'thisline' stores the atomic symbols
        {
          if (thisline[i] != satoms[i]) {
            ShowWarning("vasp", "'species' does not match with the species found in the file. "
                                "Assuming information read in the file.");
            satoms = thisline;
            break;
          }
        }
        getline(is, tmp);
        thisline = StringSplit(tmp, ' ');
        for (int i = 0; i < thisline.Size();
             ++i) // Now 'thisline' stores the amount of atoms per species
        {
          if (thisline[i] != numesp[i]) {
            ShowWarning("vasp", "'numatoms' does not match with the number of atoms per species "
                                "found in the file. Assuming information read in the file.");
            numesp = thisline;
            break;
          }
        }
      ...
      {
        if (speclist == "NULL")
          throw PluginError("vasp", "Error, you must pass a species list.");
        else
          numesp = thisline;
      }
      // 3rd case
      else if (numatoms != "NULL" && isnumber) // VASP 4.6
      {
        ...
        for (int i = 0; i < thisline.Size(); ++i) // Here, 'thisline' stores the atomic symbols
        {
          if (thisline[i] != numesp[i]) {
            ShowWarning("vasp",
                        "Number of atoms given do not match with number of atoms per species found "
                        "in the file. Number of atoms from the file assumed.");
            satoms = thisline;
            break;
          }
        }
      ...
      // Reads the type of atomic positions (Direct/Cartesian). In the case of 'selective dynamics',
      // ignore it.
      getline(is, tmp);
      RemoveUnnecessarySpaces(tmp);
      if (tmp[0] == 's' || tmp[0] == 'S')
      ...
      if (tmp[0] == 'c' || tmp[0] == 'C') {
        ...
        ShowWarning("plugin vasp",
                    "The type of cell couldn't be read correctly, type=\"Direct\" was assumed.");
    }
      ...
      if (secondline.Size() > 1) // VASP 4.6
      {
        ...
        if (numatoms == "NULL")
          ...
          for (int i = 0; i < numesp.Size(); ++i)
            n += (int)atof(numesp[i].c_str());
          int atoms = (int)atof(thisline[0].c_str());
          if (n != atoms) {
            ShowWarning("vasp", "'numatoms' does not match with the total number of atoms found in "
                                "the file. Assuming information read in the file.");
            numesp = StringSplit(thisline[0], ' ');
          }
        }
        for (int i = 0; i < 4; ++i)
          getline(is, tmp); // skip unnecesary lines
      } else                // VASP 5.2
      {
        ...
        for (int i = 0; i < thisline.Size(); ++i) {
          ...
          isnumber = (inpStream >> inpValue) ? 1 : 0;
          if (i > 0 && isnumber != isnumber0)
            throw PluginError("vasp", "Error, mixed type of variables on line 6 of XDATCAR file.");
          isnumber0 = isnumber;
        }
  ...
  bool aff =
      numesp.Size() !=
      satoms.Size(); // For poscar and contcar, check that N("Si,O")=N("16,32")=2, for example.
  if (ftype == "xdatcar")
    aff = false;

  if (aff) {
    throw PluginError("vasp", "Error, the number of species given does not match with the number "
                              "of species of the POSCAR file.");
  } else {
    long int S = numesp.Size();
    // For VASP 4.6: Assume one species if none is given.
    if (ftype == "xdatcar" && numatoms == "NULL" && secondline.Size() > 1)
      S = 1;
        ...
        if (tipo == "Cartesian") {
          ...
          for (int q = 0; q < 3; ++q)
            if (vtmp[q] < 0)
              vtmp[q] = 1 + vtmp[q];
          vtmp =
              cell.Cartesian(vtmp); // Antes era: vtmp = vtmp[0]*cv[0]+vtmp[1]*cv[1]+vtmp[2]*cv[2];
    ...
    if (ftype == "poscar" || ftype == "contcar") {
      ...
      if (tmp != "") {
        ...
        for (int j = 0; j < n; ++j) {
          if (is.eof()) {
            throw PluginError(
                "vasp", "Error, the number of velocities does not match with the number of atoms.");
            return false;
          }
          std::istringstream vst(tmp);
          vst >> x >> y >> z;
...
void VaspFormat::WriteCell(std::ostream& out, Configuration& con) const {
  ...
  if (ftype == "poscar" || ftype == "contcar") {
    ...
    long double det = 0.0;
    det += cell[0][0] * cell[1][1] * cell[2][2] + cell[1][0] * cell[2][1] * cell[0][2] +
           cell[2][0] * cell[0][1] * cell[1][2];
    det -= cell[2][0] * cell[1][1] * cell[0][2] + cell[2][1] * cell[1][2] * cell[0][0] +
           cell[2][2] * cell[1][0] * cell[0][1];
    long double A[3][3];
