...
static bool Zup = false, Zdown = false, RotUp = false, RotDown = false, RotRight = false,
            RotLeft = false;                          // Zoomer
...
static GLuint atomsid, cylID, axesID,
    zoomID;                              // display-list id's of all atoms, cylinder, axes and zoom
...
void Display::MakeAtoms() {
  ...
  for (i = 0; i < natoms; i++) {
    glPushMatrix();
    Ratom = sseg->color[0][i];
    Gatom = sseg->color[1][i];
    Batom = sseg->color[2][i]; /* RGB color of an atom */
    glTranslatef(atoms[i].crd[0], atoms[i].crd[1], atoms[i].crd[2]);
    if (atoms[i].marked == 0)
      glColor3f(Ratom, Gatom, Batom);
    else
...
Display::Display(const lpmd::Simulation& sim, SharedSegment& sharedseg) : sseg(&sharedseg) {
  ...
  for (int q = 0; q < 3; ++q)
    diag[q] = celda[0][q] + celda[1][q] + celda[2][q];
  ...
  for (long int i = 0; i < natoms; ++i) {
    for (int q = 0; q < 3; ++q)
      atoms[i].crd[q] = sim_atoms[i].Position()[q];
    atoms[i].marked = 0;
  }
  if (sseg->markedatom != -1)
    atoms[sseg->markedatom].marked = 1;
}
//-------------------------------------------------------------------------------------------------------------------//
//------------------------------------------- Destructor
//-----------------------------------------------------------//
...
void Display::SetupDisplay(int width, int height, int nplon, int nplat, double phi, double theta) {
  ...
  if (sseg->cameraobj[0] == M_PI && sseg->cameraobj[1] == M_PI && sseg->cameraobj[2] == M_PI)
    for (int q = 0; q < 3; ++q)
      center[q] = 0.5 * diag[q]; // default
  else                           // user's choice
  {
    for (int q = 0; q < 3; ++q)
      center[q] = sseg->cameraobj[q];
    point = true;
  }
  ...
  fovy = (GLdouble)(2 * atan((double)fovy) / M_PI *
                    180.0);      // Field of view touches exactly the borders of the cell
  ...
  if (x != M_PI && y != M_PI && z != M_PI) {
    ...
    for (int q = 0; q < 3; ++q)
      diff[q] = sseg->camerapos[q] - center[q];
    for (int q = 0; q < 3; ++q)
      diff2 += diff[q] * diff[q];
  ...
  camdir[2] = cos(theta);
  float phz = 0.5 * M_PI;
  /* Set 'v', a 90° rotation of the vector 'camdir' (adding 0.5*M_PI to the "theta" angle in
   * spherical coordinates) along the plane phi=const. */
  v[0] = sin(theta + phz) * cos(phi);
  v[1] = sin(theta + phz) * sin(phi);
  v[2] = cos(theta + phz);
  /* Set 'up'='camdir'x'v' vector, that indicates which direction is up (the direction from the
  ...
  if ((nlon == 0) && (nlat == 0))
    MakeAtomPoint(sphereid);
  else
    MakeFastNiceSphere(sphereid, sseg->atomrad);
...
void Display::makeCurframeGeom() {
  glNewList(atomsid, GL_COMPILE);
  if (RefFrame)
    MakeBox(Lines_Or_Quads, sseg->cell);
  if (axis)
    glCallList(axesID);
  MakeAtoms();
  glEndList();
...
void RotateVector(float* vect, float* axis, float ang) {
  ...
  double rv[3];
  rv[0] = u * (u * x + v * y + w * z) + (x * (v * v + w * w) - u * (v * y + w * z)) * cos(ang) +
          sqrt(u * u + v * v + w * w) * (-w * y + v * z) * sin(ang);
  rv[1] = v * (u * x + v * y + w * z) + (y * (u * u + w * w) - v * (u * x + w * z)) * cos(ang) +
          sqrt(u * u + v * v + w * w) * (w * x - u * z) * sin(ang);
  rv[2] = w * (u * x + v * y + w * z) + (z * (u * u + v * v) - w * (u * x + v * y)) * cos(ang) +
          sqrt(u * u + v * v + w * w) * (-v * x + u * y) * sin(ang);
...
void SetCamera(double dist) {
  ...
  if (!point)
    for (int q = 0; q < 3; ++q)
      center[q] = 0.5 * diag[q] + tx * v[q] + ty * up[q];
  else
    for (int q = 0; q < 3; ++q)
      center[q] = sseg->cameraobj[q];

  for (int q = 0; q < 3; ++q)
    eye[q] = center[q] + dist * camdir[q];
...
void SetCamera(double theta, double phi) {
  ...
  camdir[2] = cos(theta);
  float phz = 0.5 * M_PI;
  // 'v' is a 90° rotation of the vector 'camdir' (adding 0.5*M_PI to the "theta" angle in spherical
  // coordinates) along the plane phi=const.
  v[0] = sin(theta + phz) * cos(phi);
  v[1] = sin(theta + phz) * sin(phi);
  v[2] = cos(theta + phz);
  // 'up'='camdir'x'v' vector indicates which direction is up (the direction from the bottom to the
...
//------------------------------------------------ Move Camera
//-----------------------------------------------------------//
// If you call MoveCamera(0,3), the camera will move around the vertical axis +3 degrees (right-hand
// rule assumed) If you call MoveCamera(2,0), the camera will move around the horizontal axis +2
// degrees (right-hand rule assumed)
...
void DrawStatistics(void) {
  ...
  const GLfloat pos[] = {static_cast<GLfloat>(1.5 * sseg->width1),
                         static_cast<GLfloat>(-0.5 * static_cast<double>(sseg->height1)), 10.0f,
                         0.0f};
  ...
  if (wrt) {
    ...
    if (AtomSel) {
      Print(TXT, rm, "Atom Selection Enabled", SctAtm, 10, -150, 0);
    }
  ...
  if (sseg->persp)
    ...
    glOrtho(-fac * sseg->width1, fac * sseg->width1, -fac * sseg->height1, fac * sseg->height1,
            -20 * far, 20 * far);
  glMatrixMode(GL_MODELVIEW);
  glPopMatrix();
...
void DrawStatistics2(void) {
  ...
  int sep = 12;
  int sep2 = 15;
  if (Letter == GLUT_BITMAP_TIMES_ROMAN_24) {
    sep = 20;
    sep2 = 25;
  }
  ...
  Print(TXT, tq, "HELP MENU:");
  glTranslated(0, -sep, 0);
  Print(TXT, tr, "KEYBOARD OPTIONS:");
  glTranslated(10, -sep, 0);
  Print(TXT, Letter, "  'a' : Hide/show the coordinate axes.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'c' : Hide/show the simulation cell borders.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'f' : FullScreen.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'F' : Reshape the window to its normal size.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'g' : Pressed on the plots window, hide/show the grid.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'i' : Hide/show the zoom and rotations drivers.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter,
        "  'l' : Change the borders of the simulation cell between quads and contours.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'o' : Perspective selection (orthographic/perspective).");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'p' : Pause.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'q' : Exit.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'r' : Undo rotations, translations and zoom (restore the scene).");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  's' : Enable/Disable the Atom Selection Mode (see '+' and '-' keys.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  't' : Enable/Disable auto-rotation of the scene.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'z' : Begin/stop zoom in.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'Z' : Begin/stop zoom out.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter,
        "  '+' : In Atom Selection Mode, go to next atom; else, increase translation speed.");
  glTranslated(0, -sep, 0);
  Print(
      TXT, Letter,
      "  '-' : In Atom Selection Mode, go to the previous atom; else, decrease translation speed.");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter,
        "  '1'->'9' : In Atom Selection Mode, go to the atom typed (for example:'38').");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'CTRL'+'+' : Increase the Zoom speed (zoom in/out faster).");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "  'CTRL'+'-' : Decrease the Zoom speed (zoom in/out slower).");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'KeyUp', 'Key Down' : 90 deg. vertical rotations.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'KeyLeft', 'KeyRight' : 90 deg. horizontal rotations.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'SHIFT'+'KeyUp', 'SHIFT'+KeyDown  : 5 deg. vertical rotations.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'SHIFT'+'KeyLeft', 'SHIFT'+'KeyRight' : 5 deg. horizontal rotations.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'CTRL'+'KeyUp', 'CTRL'+'KeyDown' : Vertical translations.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'CTRL'+'KeyLeft', 'CTRL'+'KeyRight' : Horizontal translations.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'F1' : Open this window.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'F2' : Open the Simulation Data window.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'F3' : Open the plots window.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'RePag' : Changes fonts to next available.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  'AvPag' : Changes fonts to previous available.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "MOUSE MOTION OPTIONS.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  Left Button : Rotates the scene.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  Right Button : Display menu in any window.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  Middle Button : Zoom to the scene.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "MOUSE CLICKING OPTIONS.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter,
        "  Click on the horizontal arrows : Rotates the scene sloyly to the right/left.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter,
        "  Click on the vertical arrows in the corner : Rotates the scene slowly up/down.");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "  Click on the vertical arrows at the left : Zoom in/out.");
  glTranslated(200, -50, 0);
  Print(TXT, Letter, "Designed By:");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "- Sergio Davis:");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "- Joaquin Peralta:");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "- Claudia Loyola:");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "- Felipe Gonzalez:");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "- Pablo Ravelo:");
  glTranslated(0, -sep, 0);
  Print(TXT, Letter, "- Yasmin Navarrete:");
  glTranslated(0, -sep2, 0);
  Print(TXT, Letter, "     www.gnm.cl");
...
void DrawStatistics3(void) {
  ...
  Print(TXT, GLUT_BITMAP_9_BY_15, "LPVISUAL 2.0", sseg->width3 / 3.0, 0, 0);
  glColor3f(0, 0, 0);
  glTranslated(0, -30, 0);
  Print(TXT, Letter, "Atoms", (double)sseg->natoms, 0, 0, 0);
  if (!sseg->noprop) {
  ...
  {
    std::stringstream txt;
    txt << "CameraPosition = (" << eye[0] << ", " << eye[1] << ", " << eye[2] << ")";
  ...
  {
    std::stringstream txt;
    txt << "LookingAt      = (" << center[0] << ", " << center[1] << ", " << center[2] << ")";
    Print(TXT, Letter, txt, 0, 0, 0);
  }
  ...
  {
    std::stringstream txt;
    txt << "UpDirection    = (" << up[0] << ", " << up[1] << ", " << up[2] << ")";
    Print(TXT, Letter, txt, 0, 0, 0);
  }
  ...
  {
    std::stringstream txt;
    txt << "Window Size    =  " << sseg->width1 << " x " << sseg->height1;
    Print(TXT, Letter, txt, 0, 0, 0);
  }
...
void CreateWIN1() {
  ...
  InitWIN1();
  if (!sseg->paused)
    glutSetWindowTitle("LPVisual");
  else
    glutSetWindowTitle("LPVisual (paused)");
...
void CheckUpdates(void) {
  ...
  if (z_held_down) {
    if (sseg->persp) {
      distance -= 0.05 * diagonal * zoom_factor;
      markerZ = -40.0 / diagonal * distance + 40;
    } else {
      orth_dist -= 0.05 * diagonal * zoom_factor;
    ...
    ty -= 0.01 * gap * diagonal;
    MoveCamera(0, 0);
  } else if (d_arrow_down) {
    ty += 0.01 * gap * diagonal;
    MoveCamera(0, 0);
  } else if (r_arrow_down) {
    tx -= 0.01 * gap * diagonal;
    MoveCamera(0, 0);
  ...
  if (sseg->mustUpdate) {
    ...
    for (int q = 0; q < 3; ++q)
      diag[q] = sseg->cell[0][q] + sseg->cell[1][q] + sseg->cell[2][q];
    MoveCamera(0, 0);
...
void CheckKeyboard(unsigned char key, int x, int y) {
  ...
  if (glutGetWindow() == sseg->WIN1) {
    ...
    case 'q': {
      ...
      if (sseg->liveWIN2)
        glutDestroyWindow(sseg->WIN2);
      if (sseg->liveWIN3)
        glutDestroyWindow(sseg->WIN3);
      if (sseg->liveWIN4)
        glutDestroyWindow(sseg->WIN4);
    } break;
    // RESTORE THE SCENE
    case 'r': {
      tx = 0;
    ...
    // AUTO-ROTATE
    case 't':
      sseg->autorot = !sseg->autorot;
      break;
    // UNZOOM
    case 'z': {
      z_held_down = !z_held_down;
      Z_held_down = false;
    } break;
    // ZOOM
    case 'Z': {
...
void CheckDirectionalKeys(int key, int x, int y) {
  ...
  if (glutGetWindow() == sseg->WIN1) {
    ...
    case GLUT_KEY_PAGE_UP: {
      ...
      switch (op) {
      case B9_BY_15:
        Letter = GLUT_BITMAP_9_BY_15;
        break;
      case B8_BY_13:
        Letter = GLUT_BITMAP_8_BY_13;
        break;
      case TIMES_ROMAN_10:
        Letter = GLUT_BITMAP_TIMES_ROMAN_10;
        break;
      case TIMES_ROMAN_24:
        Letter = GLUT_BITMAP_TIMES_ROMAN_24;
        break;
      case HELVETICA_10:
        Letter = GLUT_BITMAP_HELVETICA_10;
        break;
      case HELVETICA_12:
        Letter = GLUT_BITMAP_HELVETICA_12;
        break;
      case HELVETICA_18:
        Letter = GLUT_BITMAP_HELVETICA_18;
        break;
      default: {
        op = B9_BY_15;
        Letter = GLUT_BITMAP_9_BY_15;
...
void CheckMouse(int button, int state, int x, int y) {
  ...
  if (glutGetWindow() == sseg->WIN1) {
    // MANAGE THE ZOOMER
    if (Buttons[0]) {
      gluUnProject(mousecoord[0], sseg->height1 - mousecoord[1], 0, model_view, projection,
      ...
      if (pos3D_x > orgX && pos3D_x < orgX + 30 && pos3D_y > orgY && pos3D_y < orgY + 22)
        RotUp = true;
      // If you press the bottom arrow, shine it
      else if (pos3D_x > orgX && pos3D_x < orgX + 30 && pos3D_y > orgY - 115 &&
               pos3D_y < orgY - 115 + 22)
        RotDown = true;
      // If you press the left arrow, shine it
      else if (pos3D_x > orgX - 103 && pos3D_x < orgX - 103 + 22 && pos3D_y > orgY - 155 &&
               pos3D_y < orgY - 155 + 30)
        RotLeft = true;
      // If you press the right arrow, shine it
      else if (pos3D_x > orgX - 103 + 137 && pos3D_x < orgX - 103 + 22 + 137 &&
               pos3D_y > orgY - 155 && pos3D_y < orgY - 155 + 30)
        RotRight = true;
      // If you press the top arrow of the zoom, shine it
      else if (pos3D_x > orgX - 60 && pos3D_x < orgX - 30 && pos3D_y > orgY && pos3D_y < orgY + 22)
        Zup = true;
      // If you press the bottom arrow of the zoom, shine it
      else if (pos3D_x > orgX - 60 && pos3D_x < orgX - 30 && pos3D_y > orgY - 115 &&
               pos3D_y < orgY - 115 + 22)
        Zdown = true;
      // If you press the angles (vertical text), change to that angle
      else if (pos3D_x > orgX + 30 && pos3D_x < orgX + 50 && pos3D_y > orgY - 90 &&
               pos3D_y < orgY) {
        if (pos3D_y < orgY && pos3D_y > orgY - 10)
          marker = 40;
        else if (pos3D_y < orgY - 10 && pos3D_y > orgY - 20)
          marker = 30;
        else if (pos3D_y < orgY - 20 && pos3D_y > orgY - 30)
          marker = 20;
        else if (pos3D_y < orgY - 30 && pos3D_y > orgY - 40)
          marker = 10;
        else if (pos3D_y < orgY - 40 && pos3D_y > orgY - 50)
          marker = 0;
        else if (pos3D_y < orgY - 50 && pos3D_y > orgY - 60)
          marker = -10;
        else if (pos3D_y < orgY - 60 && pos3D_y > orgY - 70)
          marker = -20;
        else if (pos3D_y < orgY - 70 && pos3D_y > orgY - 80)
          marker = -30;
        else if (pos3D_y < orgY - 80 && pos3D_y > orgY - 90)
          marker = -40;
        else
          return; // exit CheckMouse if you didn't hit the text
      ...
      else if (pos3D_x > orgX - 90 && pos3D_x < orgX + 60 && pos3D_y > orgY - 165 &&
               pos3D_y < orgY - 115) {
        if (pos3D_x > orgX - 87 && pos3D_x < orgX - 64 && pos3D_y > orgY - 165 &&
            pos3D_y < orgY - 150)
          marker2 = -50;
        else if (pos3D_x > orgX - 75 && pos3D_x < orgX - 50 && pos3D_y > orgY - 130 &&
                 pos3D_y < orgY - 115)
          marker2 = -37.5;
        else if (pos3D_x > orgX - 62 && pos3D_x < orgX - 39 && pos3D_y > orgY - 165 &&
                 pos3D_y < orgY - 150)
          marker2 = -25;
        else if (pos3D_x > orgX - 48 && pos3D_x < orgX - 26 && pos3D_y > orgY - 130 &&
                 pos3D_y < orgY - 115)
          marker2 = -12.5;
        else if (pos3D_x > orgX - 28 && pos3D_x < orgX - 16 && pos3D_y > orgY - 165 &&
                 pos3D_y < orgY - 150)
          marker2 = 0;
        else if (pos3D_x > orgX - 18 && pos3D_x < orgX - 3 && pos3D_y > orgY - 130 &&
                 pos3D_y < orgY - 115)
          marker2 = 12.5;
        else if (pos3D_x > orgX - 7 && pos3D_x < orgX + 13 && pos3D_y > orgY - 165 &&
                 pos3D_y < orgY - 150)
          marker2 = 25;
        else if (pos3D_x > orgX + 5 && pos3D_x < orgX + 25 && pos3D_y > orgY - 130 &&
                 pos3D_y < orgY - 115)
          marker2 = 37.5;
        else if (pos3D_x > orgX + 16 && pos3D_x < orgX + 36 && pos3D_y > orgY - 165 &&
                 pos3D_y < orgY - 150)
          marker2 = 50;
        else
          return; // exit CheckMouse if you didn't hit the text
      ...
      else if (pos3D_x > orgX - 50 && pos3D_x < orgX - 35 && pos3D_y > orgY - 50 &&
               pos3D_y < orgY - 40) {
        markerZ = 0;
        if (sseg->persp)
          distance = sqrt(diag[0] * diag[0] + diag[1] * diag[1] + diag[2] * diag[2]);
        else
          orth_dist = sqrt(diag[0] * diag[0] + diag[1] * diag[1] + diag[2] * diag[2]);
        ;
        MoveCamera(0, 0);
      }
...
//----------------------------------------------- Display1
//-----------------------------------------------------------//
// Callback for glutDisplayFunc().  It clears the frame and depth buffers and draws the atoms in the
// current frame.
void Display1(void) {
  SharedSegment* sseg = (SharedSegment*)(shpointer);
  ...
  else if (fondo == "orange")
    glClearColor(1.0, 0.27, 0.0, 0.0);
  else
  ...
  gluLookAt((GLdouble)eye[0], (GLdouble)eye[1], (GLdouble)eye[2], (GLdouble)center[0],
            (GLdouble)center[1], (GLdouble)center[2], (GLdouble)up[0], (GLdouble)up[1],
            (GLdouble)up[2]);
...
void Display2(void) {
  ...
  glPopMatrix();
  glPushMatrix();
  glTranslated(2 * cs, 2 * cs, 0);
  glRotatef(sseg->mdstep, 1, 0, 0);
  glutSolidCube(cs);
  glPopMatrix();
  // The "D"
  glTranslated(4 * cs, 0, 0);
  glColor3d(0.5, 0.5, 0);
  glPushMatrix();
  glRotatef(sseg->mdstep, 0.1, 1, .1);
  glutSolidCube(cs);
...
void Display4(void) {
  ...
  if (sseg->numyrange == 0) {
    for (int i = 0; i < sseg->numplots; ++i) {
      if (sseg->data[i][sseg->mdstep] > sseg->yrange[1] ||
          sseg->data[i][sseg->mdstep] < sseg->yrange[0]) {
        sseg->yrange[0] *= 2;
        sseg->yrange[1] *= 2;
      }
    }
  ...
  if (sseg->yrange[0] * sseg->yrange[1] < 0)
    axisxpos = 0;
  else if (sseg->yrange[1] > 0)
    axisxpos = sseg->yrange[0];
  else if (sseg->yrange[1] <= 0)
    axisxpos = sseg->yrange[1];

  //++++++++++ WINDOW GEOMETRY +++++++++++//
  glOrtho(sseg->xrange[0] - 0.15 * xsep, sseg->xrange[1] + 0.2 * xsep, sseg->yrange[0] - 0.2 * ysep,
          sseg->yrange[1] + 0.2 * ysep, -10, 10);
  glMatrixMode(GL_MODELVIEW);
  ...
  //+++++ AXES ++++++++++//
  glBegin(GL_LINES);
  if (fondo != "black" && fondo != "orange" && fondo != "gray")
    glColor3f(0, 0, 0);
  else
    glColor3f(0, 1, 0);
  // X-Axis
  glVertex2d(-sseg->xrange[1], axisxpos);
  glVertex2d(sseg->xrange[1] + 0.1 * xsep, axisxpos);
  // Y-Axis
  if (sseg->yrange[0] * sseg->yrange[1] < 0) {
    glVertex2d(axisypos, 2 * sseg->yrange[0]);
    glVertex2d(axisypos, sseg->yrange[1] + 0.1 * ysep);
  } else if (sseg->yrange[1] > 0) {
    glVertex2d(axisypos, -sseg->yrange[0]);
    glVertex2d(axisypos, sseg->yrange[1] + 0.1 * ysep);
  } else {
    glVertex2d(axisypos, 2 * sseg->yrange[0]);
    glVertex2d(axisypos, sseg->yrange[1] + 0.1 * ysep);
  ...
  for (int i = -10; i <= 10; ++i) {
    // Y-Ticks
    glBegin(GL_LINES);
    glVertex2d(axisypos - 0.01 * xsep,
               (i - 10) * (sseg->yrange[1] - sseg->yrange[0]) / 20.0 + sseg->yrange[1]);
    glVertex2d(axisypos + 0.01 * xsep,
               (i - 10) * (sseg->yrange[1] - sseg->yrange[0]) / 20.0 + sseg->yrange[1]);
    glEnd();
  }
  ...
  // X-Labels
  for (int i = -10; i <= 10; ++i) {
    std::stringstream Xtxt;
    Xtxt << (i / 10.0) * sseg->xrange[1];
    Print(TXT, GLUT_BITMAP_HELVETICA_10, Xtxt,
          (i / 10.0) * sseg->xrange[1] - 0.02 * sseg->xrange[1], axisxpos - 0.05 * ysep, 0);
  }
  // Y-Labels
  for (int i = -10; i <= 10; ++i) {
    std::stringstream Ytxt;
    Ytxt << (i - 10) * (sseg->yrange[1] - sseg->yrange[0]) / 20.0 + sseg->yrange[1];
    if (fabs((i - 10) * (sseg->yrange[1] - sseg->yrange[0]) / 20.0 + sseg->yrange[1]) > 0.001)
      Print(TXT, GLUT_BITMAP_HELVETICA_10, Ytxt, axisypos - 0.08 * xsep,
            (i - 10) * (sseg->yrange[1] - sseg->yrange[0]) / 20.0 + sseg->yrange[1], 0);
    else
  ...
  for (int i = 0; i < sseg->numplots; ++i) {
    ...
    Print(TXT, GLUT_BITMAP_HELVETICA_18, sseg->ptname[i], axisypos + 0.05 * xsep,
          (sseg->yrange[1] + 0.1 * ysep) - 0.08 * i * ysep, 0);
  }
