...
void NoseHoover::Initialize(Simulation& sim, Potential& p) {
  for (long int i = 0; i < sim.Atoms().Size(); ++i)
...
void NoseHoover::AdvancePosition(Simulation& sim, long i) {
  ...
  friction +=
      ((3.0 * sim.Atoms().Size() / q) * kboltzmann * (Temperature(sim.Atoms()) - temp) * dt);
  const Vector newacc = now.Acceleration() - now.Velocity() * friction;
  sim.Atoms()[i].Acceleration() = newacc;
  Vector newpos = now.Position() + now.Velocity() * dt + (2.0 / 3.0) * newacc * dt * dt -
                  (1.0 / 6.0) * old.Acceleration() * dt * dt;
...
void NoseHoover::AdvanceVelocity(Simulation& sim, long i) {
  ...
  Atom now = sim.Atoms()[i];
  Atom old = oldsc.Atoms()[i];
  sim.Atoms()[i].Velocity() = now.Velocity() + (1.0 / 3.0) * now.Acceleration() * dt +
                              (5.0 / 6.0) * old.Acceleration() * dt - (1.0 / 6.0) * auxlist[i] * dt;
}
