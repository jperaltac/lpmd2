...
double fover(double r) { return (1.0 - 0.5 * r - r * r / 8.0 + r * r * r / 16.0); }
...
void SearchFill::Apply(Configuration& con) {
  ...
  long int nvac = 0;
  if (boundary != 0) {
    int risk = 0; // The vacancy position it's risky because it is close to a surface 'taged' atom.
                  // 0 = non risky.
    std::set<int> risk_id;
    risk_id.clear();
    risk_id.insert(-1);
    ...
    while (nvac < VACMAX) {
      ...
      risk = 0;
      //***************************************************************************************************************
      // Search a nice place to start(minimum overlap of an atom in the cell). Overlap must be major
      // than abs_min.****** This is the initial position to start looking
      // around.**********************************************************
      //***************************************************************************************************************
      int coord = 0;
      int nnan = 0;
      ...
      if (drand48() < 0.05) {
        for (unsigned long int i = 0; i < iN; ++i) {
          ...
          if (it == risk_id.end()) {
            ...
            if ((GoodEn > 0.0e0) && (to < GoodEn))
              break; // test criterion if apply
          }
        }
      } else {
        srand(time(0));
      ...
      r1 = r0;
      // rellocate the abs_min. If we don't do that the new atom (vacancy) will have the minimum
      // overlap now. The actual total overlap is the minimum overlap.
      abs_min = minov;
      to = minov;
      //********************************************************************************************************
      // If there is a big vaccum region some atoms could have 'to' close to
      // zero.******************************* In those cases, we will not pass thru the while
      // sentence below, so we need to re-alocate the position ** before go inside the while. If we
      // don't do this, we will add a vacancy overlapping the original atom**** with lower
      // 'to'.****************************************************************************************
      //********************************************************************************************************
      if (to <= VACOVP) {
      ...
      // Now we can really aprove this new position like a real vacancy and not overlap with an
      // original atom.
      int iter = 0;
      bool accept;
      ...
      while ((to > VACOVP) && (iter < MaxSteps) && risk <= 1) {
        ...
        coord = 0;
        new_risk = 0;
        nnan = 0; // number of non-atomic neighbors (at least one atom of the original cell must be
                  // a neighbor).
        for (unsigned long int j = 0; j < nlist.Size(); ++j) {
          const lpmd::AtomPair& nn = nlist[j];
          int nnz = nn.j->Z();
      ...
      if ((to < VACOVP && risk - new_risk == 0)) {
        if ((coord >= min_coord && coord <= max_coord) && nnan != coord) {
          std::cout << "-> Found a Vacancy! (ov = " << to << ", coord = " << coord
                    << " ) = " << vce.Position() << '\n';
          part.Append(vce);
          con.Update();
          nvac++;
