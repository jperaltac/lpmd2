...
BoxFilter::BoxFilter(std::string args) : Plugin("box", "1.0"), selector(0) {
  ...
  if (params["x"] == "all") {
    ...
    if (sx.Size() != 2)
      throw PluginError("box", "Invalid x range");
    x[0] = atof(sx[0].c_str());
    x[1] = atof(sx[1].c_str());
  }
  ...
  if (params["y"] == "all") {
    ...
    if (sy.Size() != 2)
      throw PluginError("box", "Invalid y range");
    y[0] = atof(sy[0].c_str());
    y[1] = atof(sy[1].c_str());
  }
  ...
  if (params["z"] == "all") {
    ...
    if (sz.Size() != 2)
      throw PluginError("box", "Invalid z range");
    z[0] = atof(sz[0].c_str());
    z[1] = atof(sz[1].c_str());
  }
}
...
Selector<BasicParticleSet>& BoxFilter::CreateSelector() {
  ...
  if (x[0] < 0)
    ...
    minx = fabs(x[0] * cos(aa));
  maxx = fabs(x[1] * cos(aa));
  if (y[0] < 0)
    miny = y[0];
  else
    miny = fabs(y[0] * cos(ab));
  maxy = fabs(y[1] * cos(ab));
  if (z[0] < 0)
    minz = z[0];
  else
    minz = fabs(z[0] * cos(ac));
