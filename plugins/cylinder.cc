...
void CylinderFilter::ShowHelp() const {
  ...
  std::cout << " apply tempscaling each=1 start=1 end=-1 over cylinder origin=<5,5,5> "
               "endpoint=<6,6,6> rmax=10 rmin=0\n\n";
...
Selector<BasicParticleSet>& CylinderFilter::CreateSelector() {
  ...
  lpmd::Vector a = (*mycell)[0];
  double ma = (*mycell)[0].Module();
  lpmd::Vector b = (*mycell)[1];
  double mb = (*mycell)[1].Module();
  lpmd::Vector c = (*mycell)[2];
  double mc = (*mycell)[2].Module();
