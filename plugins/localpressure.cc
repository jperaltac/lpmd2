...
void LocalPressure::Evaluate(Configuration& sim, Potential& pot) {
  ...
  try {
    p_array_ptr = dynamic_cast<CombinedPotential*>(&pot);
  } catch (std::exception&) {
    throw PluginError("localpressure",
                      "Potentials are not active, cannot compute individual stress tensors");
  }
  ...
  for (int p = 0; p < 3; ++p)
    for (int q = 0; q < 3; ++q)
      m.SetLabel(3 + (3 * p + q), "S" + ToString<int>(p + 1) + ToString<int>(q + 1));
  ...
  for (long int i = 0; i < atoms.Size(); ++i) {
    ...
    for (int j = 0; j < nlist.Size(); ++j) {
      try {
        ...
        int s2 = (nn.j)->Z();
        const PairPotential& ppot =
            dynamic_cast<const PairPotential&>(p_array.PotentialForElements(s1, s2));
        Vector ff = ppot.pairForce(nn.rij);
        for (int p = 0; p < 3; ++p)
          for (int q = 0; q < 3; ++q)
            stress[p][q] -= (nn.rij)[p] * ff[q];
      } catch (std::exception&) {
        throw PluginError("localpressure",
                          "Cannot calculate local stress with a non-pair potential.");
      }
    }
