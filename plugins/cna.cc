...
public:
  BondedPair(unsigned int j0, unsigned int k0, unsigned int l0, unsigned long int ii,
             unsigned long int jj, double r0, const Vector& c)
      : IndexTrio(j0, k0, l0), ati(ii), atj(jj), r(r0), center(c) {}
...
void CommonNeighborAnalysis::ShowHelp() const {
  ...
  std::cout
      << "         l: is the length of the longest continuous chain formed by the k bonds\n\n";
  ...
  std::cout
      << "                   57.143% (4/7) of the pairs are 6-6-6 type.                  \n\n";
...
std::vector<long int> AddSegmentToChain(std::vector<long int>& cnm,
                                        std::map<long int, std::vector<long int>>& bonds,
                                        std::vector<long int>& chain,
  ...
  for (unsigned int i = 0; i < bonds[tail].size(); ++i) {
    ...
    if ((chain.size() > 2) && (bonds[tail][i] == chain[0]))
      inchain = false; // exception to allow a closed chain
    if (!inchain) {
      flag = true;
...
void CommonNeighborAnalysis::Evaluate(Configuration& conf, Potential& pot) {
  ...
  for (long i = 0; i < n; ++i) {
    ...
    for (long int it = 0; it < nlist.Size(); ++it) {
      ...
      for (long int jt = 0; jt < jlist.Size(); ++jt) {
        ...
        if (jnn.r2 >= rcut * rcut)
          continue;
        if ((indices[jnn.j] != i) && (indices[jnn.j] != j))
          cn.push_back(indices[jnn.j]);
      }
      ...
      if (cna_indices[0] > 0) {
        ...
        for (unsigned int p = 0; p < cna_indices[0] - 1; ++p)
          for (unsigned int q = p + 1; q < cna_indices[0]; ++q) {
            if (cell.Displacement(atoms[cnm[p]].Position(), atoms[cnm[q]].Position()).Module() <
                rcut) {
        ...
        for (unsigned int p = 0; p < cna_indices[0]; ++p) {
          ...
          if (nt.size() > longest.size())
            longest = nt;
        }
        cna_indices[2] = longest.size() - 1;
      }
      data.push_back(BondedPair(cna_indices[0], cna_indices[1], cna_indices[2], i, indices[nn.j],
                                sqrt(nn.r2), atoms[i].Position() + nn.rij * 0.5));
    }
  }
  ...
  if (mode == 0) {
    ...
    *m = Matrix(7, npairs);
    ...
    for (unsigned long int q = 0; q < npairs; ++q) {
      std::string s = ToString<int>(data[q].j) + "-" + ToString<int>(data[q].k) + "-" +
                      ToString<int>(data[q].l);
      if (stat.count(s) == 0) {
        stat[s] = 0;
        stat_rav[s] = 0.0;
        stat_r2av[s] = 0.0;
    ...
    int nkeys = 0;
    for (std::map<std::string, unsigned long int>::const_iterator qt = stat.begin();
         qt != stat.end(); ++qt)
      nkeys++;
    *m = Matrix(6, nkeys);
    m->SetLabel(0, "j");
    ...
    if ((stat.size() == 1) && ((*(stat.begin())).first == "4-2-1"))
      ...
      lista.Append((*(++it)).first);
      if ((lista.Find("4-2-1") >= 0) && (lista.Find("4-2-2") >= 0))
        DebugStream() << "-> Found 4-2-1 and 4-2-2 pairs, looks like HCP\n";
      if ((lista.Find("6-6-6") >= 0) && (lista.Find("4-4-4") >= 0))
        DebugStream() << "-> Found 6-6-6 and 4-4-4 pairs, looks like BCC\n";
    } else
      DebugStream() << "-> Found " << stat.size() << " kinds of pairs, cannot tell the structure\n";
  } else if (mode == 2) {
    // Defects mode
    ...
    for (long int q = 0; q < atoms.Size(); ++q) {
      at = atoms[q];
      regcnt[q] = defcnt[q] = 0;
      if ((atoms.Have(at, Tag(filterout)) && (atoms.GetTag(at, Tag(filterout)) == "true")) ||
          (filterout == "all"))
        nfil++;
    }
    for (unsigned long int q = 0; q < npairs; ++q) {
