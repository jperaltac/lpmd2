...
LAMMPSFormat::LAMMPSFormat(std::string args) : Plugin("lammps", "1.0") {
  ...
  if (strcmp(q.c_str(), "dump") == 0) {
    ftype = "dump";
  } else if (strcmp(q.c_str(), ".data") == 0) {
    ftype = "data";
  }
...
void LAMMPSFormat::ShowHelp() const {
  ...
  std::cout
      << "      datatype      : When ftype = data, you have to select the LAMMPS atom style\n";
  std::cout << "                      used by the data file.                                   \n";
  std::cout
      << "                     (angle / atomic / bond / charge / dipole / full / molecular)\n";
  std::cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n";
  std::cout << " Example           >>                                                          \n";
...
void LAMMPSFormat::ReadHeader(std::istream& is) const {
  ...
  if (lspl[0] == "ITEM:") {
    ...
    bool found = false;
    for (int k = 0; k < lspl.Size(); ++k)
      if (lspl[k] == "element")
        found = true;
    if (!found && species[0] == "NULL")
      throw PluginError("lammps", "Error, you must give a species list.");
...
void LAMMPSFormat::ReadHeaderData(std::istream& is) const {
  ...
  if (datatype == "NULL") {
    ShowWarning("lammps", "No datatype specified. Assuming 'datatype = atomic'.");
    datatype = "atomic";
  }
  ...
  while (lspl[0] != "Atoms" && !is.eof()) {
    ...
    if (lspl.Size() == 2) {
      if (lspl[1] == "atoms")
        natoms = atoi(lspl[0].c_str());
    } else if (lspl.Size() == 4) {
      if (lspl[3] == "xhi") {
        if ((*this)["replacecell"] == "true") {
          double xlo, xhi, ylo, yhi, zlo, zhi, xy = 0.0, xz = 0.0, yz = 0.0;
          xlo = atof(lspl[0].c_str());
          xhi = atof(lspl[1].c_str());
          getline(is, line);
          lspl = StringSplit(line, ' ');
          (*linecounter)++;
          hdr.Append(line);
          ylo = atof(lspl[0].c_str());
          yhi = atof(lspl[1].c_str());
          getline(is, line);
          lspl = StringSplit(line, ' ');
          (*linecounter)++;
          hdr.Append(line);
          zlo = atof(lspl[0].c_str());
          zhi = atof(lspl[1].c_str());
          getline(is, line);
          lspl = StringSplit(line, ' ');
          (*linecounter)++;
          hdr.Append(line);
          if (lspl.Size() == 6) {
            xy = atof(lspl[0].c_str());
            xz = atof(lspl[1].c_str());
            yz = atof(lspl[2].c_str());
          }
  ...
  if (natoms == 0)
    throw PluginError("lammps",
                      "File " + readfile +
                          " doesn't seem to be in LAMMPS format (not a LAMMPS dump/data file)");
...
bool LAMMPSFormat::ReadCell(std::istream& is, Configuration& con) const {
  ...
  if (ftype == "dump") {
    ...
    if (words.Size() == 2) {
      ...
      if ((*this)["replacecell"] == "true") {
        xlo = atof(words[0].c_str());
        xhi = atof(words[1].c_str());
        xy = atof(words[2].c_str());
        getline(is, tmp);
        words = StringSplit(tmp, ' ');
        (*linecounter)++;
        ylo = atof(words[0].c_str());
        yhi = atof(words[1].c_str());
        xz = atof(words[2].c_str());
        getline(is, tmp);
        words = StringSplit(tmp, ' ');
        (*linecounter)++;
        zlo = atof(words[0].c_str());
        zhi = atof(words[1].c_str());
        yz = atof(words[2].c_str());
        if (xy > 0)
          xhi -= xy;
        else
          xlo -= xy;
      ...
      throw PluginError("lammps", "Error ocurred when reading box dimensions, file \"" + readfile +
                                      "\", line " + ToString<int>(*linecounter));

    getline(is, tmp); // Something like "ITEM: ATOMS id type xu yu zu vx vy vz c_poteng"
    words = StringSplit(tmp, ' ');
    (*linecounter)++;
    if (words[1] != "ATOMS")
      throw PluginError("lammps", "Error ocurred after reading box dimensions, file \"" + readfile +
                                      "\", line " + ToString<int>(*linecounter));
    hdr.Clear();
    ...
    for (long int k = 2; k < hdr.Size(); ++k)
      if (hdr[k][0] == 'x' || hdr[k][0] == 'y')
        coords = true;
    if (!coords)
      throw PluginError("lpmd", "Coordinates x y z not found in line " +
                                    ToString<int>(*linecounter) + ": \"" + tmp + "\"");
  }
  ...
  for (long int i = 0; i < natoms; ++i) {
    ...
    words = StringSplit(tmp, ' ');
    if (words.Size() == 0)
      throw PluginError("lammps", "Error ocurred, the input file does not have elements!");
      ...
      if (ftype == "dump") {
        for (long int k = 2; k < hdr.Size(); ++k) {
          if (hdr[k] == "id")
            id = atoi(words[k - 2].c_str());
          else if (hdr[k] == "type")
            spec_num = atoi(words[k - 2].c_str());
          else if (hdr[k] == "element")
            elem = words[k - 2].c_str();
          else if (hdr[k][0] == 'x') {
            X = atof(words[k - 2].c_str());
            if (hdr[k][1] == 's')
              shrink_wrapped = true;
          } else if (hdr[k][0] == 'y') {
            Y = atof(words[k - 2].c_str());
            if (hdr[k][1] == 's')
              shrink_wrapped = true;
          } else if (hdr[k][0] == 'z') {
            Z = atof(words[k - 2].c_str());
            if (hdr[k][1] == 's')
              shrink_wrapped = true;
          } else if (hdr[k] == "vx")
            VX = atof(words[k - 2].c_str());
          else if (hdr[k] == "vy")
            VY = atof(words[k - 2].c_str());
          else if (hdr[k] == "vz")
            VZ = atof(words[k - 2].c_str());
        }
        pos = Vector(X, Y, Z);
        vel = Vector(VX, VY, VZ);
        if (shrink_wrapped)
          pos = cell.Cartesian(Vector(X, Y, Z));
      } else if (ftype == "data") {
        ...
        if (words.Size() == 5) {
          ...
          c_y = 3;
          c_z = 4;
        } else if (words.Size() == 6 && datatype == "molecular") {
          c_id = 0;
          c_sn = 2;
          c_x = 3;
          c_y = 4;
          c_z = 5;
        } else if (words.Size() == 6 && datatype == "charge") {
          c_id = 0;
          c_sn = 1;
          c_x = 3;
          c_y = 4;
      ...
      if (spec_num > species.Size())
        throw PluginError("lammps", "Missing species in \"" + readfile + "\", line " +
                                        ToString<int>(*linecounter) + ". Only " +
                                        ToString<int>(species.Size()) + " given, but more than " +
                                        ToString<int>(species.Size()) + " were found.");
      ...
      throw PluginError("lammps", "An unidentified line was found in the file \"" + readfile +
                                      "\", line " + ToString<int>(*linecounter));
  }
  ...
  if (displace == "true") {
    if (ftype == "data") {
      words = StringSplit(hdr[3], ' ');
      xlo = atof(words[0].c_str());
      words = StringSplit(hdr[4], ' ');
      ylo = atof(words[0].c_str());
      words = StringSplit(hdr[5], ' ');
      zlo = atof(words[0].c_str());
    }
    for (int i = 0; i < natoms; i++)
      part[i].Position() -= Vector(xlo, ylo, zlo);
  }
  ...
  if (ftype == "dump") {
    ...
    if (tmp == "Velocities") {
      ...
      is.seekg(len, std::ios_base::beg); // Go to the beggining of the block
    ...
    if (tmp == hdr[0])
      ...
      while (!is.eof())
        getline(is, tmp); // Ignore everything else after velocities
    }
  }
...
void LAMMPSFormat::WriteHeader(std::ostream& os, SimulationHistory* sh) const {
  ...
  if (ftype != "dump" && ftype != "data")
    throw PluginError("lammps", "File format ftype must be either 'dump' or 'data'");
  (*linecounter) = 0;
}
...
void LAMMPSFormat::WriteCell(std::ostream& out, Configuration& con) const {
  ...
  if (ftype == "dump") {
    ...
    for (long int i = 0; i < part.Size(); ++i)
      min = (part[i].ID() < min) ? part[i].ID() : min;
    min--;

    if (level == 0) {
      out << "ITEM: ATOMS id type x y z" << std::endl;
      for (long int i = 0; i < part.Size(); i++)
        out << part[i].ID() - min << " " << elemtype[part[i].Z()] << " " << part[i].Position()
            << std::endl;
    } else if (level == 1) {
      out << "ITEM: ATOMS id type x y z vx vy vz" << std::endl;
      for (long int i = 0; i < part.Size(); i++)
        out << part[i].ID() - min << " " << elemtype[part[i].Z()] << " " << part[i].Position()
            << " " << part[i].Velocity() << std::endl;
    } else {
    ...
    out << "\nMasses\n\n";
    for (long int i = 0; i < esp.Size(); i++)
      out << elemtype[esp[i]] << " " << at[i].Mass() << " # " << at[i].Symbol() << std::endl;
    ...
    if (level == 1) {
      out << "\nVelocities\n\n";
      for (long int i = 0; i < part.Size(); i++)
        out << part[i].ID() - min << " " << part[i].Velocity() << std::endl;
    }
  }
}
