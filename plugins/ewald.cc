  ...
  for (int pp = 0; pp <= nrep; ++pp)
    for (int qq = (pp == 0 ? 0 : -nrep); qq <= nrep; ++qq)
      for (int rr = (pp == 0 && qq == 0 ? 1 : -nrep); rr <= nrep; ++rr) {
        ...
        if ((fabs(k.Module()) > 1.0E-10) && (k.Module() < kcut))
          kpoints->push_back(k);
      }
  kfac = new double[kpoints->size()];
  for (unsigned int nk = 0; nk < kpoints->size(); ++nk) {
    Vector& tmp = (*kpoints)[nk];
    kfac[nk] = 4.0 * M_PI * (1.0 / tmp.SquareModule()) * (1.0 / cell.Volume()) *
               exp(-tmp.SquareModule() / (4.0 * alpha * alpha));
  }
  ...
  for (long int i = 0; i < atoms.Size(); ++i) {
    ...
    for (long int j = 0; j < nlist.Size(); ++j) {
      ...
      if (nn.r2 < rcut * rcut) {
        ...
        ff = (nn.rij) * (1.0 / rmod);
        ff = ff * qi * qj *
             (erfc(alpha * rmod) / rmod +
              2.0 * (alpha / sqrt(M_PI)) * exp(-alpha * alpha * rmod * rmod));
        ff = ff * (-1.0 / rmod);
  ...
  for (unsigned int nk = 0; nk < kpoints->size(); ++nk) {
    ...
    for (long int i = 0; i < atoms.Size(); ++i) {
      ...
      atoms[i].Acceleration() =
          atoms[i].Acceleration() +
          (k * 2.0 * qi * kfac[nk] * (sin(dkr[i]) * sumqcos - cos(dkr[i]) * sumqsin)) * Q2a2FORCE /
              atoms[i].Mass();
    }
...
double Ewald::EnergyConstantCorrection(Configuration& conf) {
  ...
  double e = 0.0;
  for (long int i = 0; i < atoms.Size(); ++i)
    e += pow(atoms[i].Charge(), 2.0);
  e *= -alpha * (1.0 / sqrt(M_PI));
  double totch = 0.0;
  for (long int i = 0; i < atoms.Size(); ++i)
    totch += atoms[i].Charge();
  e -= pow(totch, 2.0) * M_PI / (4.0 * cell.Volume() * alpha * alpha);
  return e * Q2a2EV;
}

double Ewald::energy(Configuration& conf) {
  if (conf.HaveAny(Tag("pe")))
    return double(Parameter(conf.GetTag(conf, Tag("pe"))));
...
void Ewald::UpdateForces(Configuration& conf) {
  ...
  if (surfdip) {
    SurfaceDipole(conf, forces, edip);
  }
