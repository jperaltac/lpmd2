...
void DRBeeman::AdvancePosition(Simulation& sim, long i) {
  ...
  Vector newpos = now.Position() + now.Velocity() * dt +
                  (2.0 / 3.0) * now.Acceleration() * dt * dt -
                  (1.0 / 6.0) * old.Acceleration() * dt * dt;
  if ((newpos - pos).Module() >= distance)
    newpos = pos;
  for (int i = 0; i < 3; ++i) {
    double np = newpos[i];
    if (np > cell[i].Module() || np < 0)
      newpos[i] = pos[i];
  }
...
void DRBeeman::AdvanceVelocity(Simulation& sim, long i) {
  ...
  const Atom& now = atoms[i];
  const Atom& old = oldatoms[i];
  atoms[i].Velocity() = now.Velocity() + (1.0 / 3.0) * now.Acceleration() * dt +
