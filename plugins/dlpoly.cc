...
void SafeFormat(char (&buffer)[512], const char* format, Args... args) {
  const int written = std::snprintf(buffer, sizeof(buffer), format, args...);
  if (written < 0 || static_cast<std::size_t>(written) >= sizeof(buffer))
    throw PluginError("dlpoly", "Formatted output exceeds the 512-byte buffer");
}
} // namespace
...
DlPolyFormat::DlPolyFormat(std::string args) : Plugin("dlpoly", "2.0"), dt(0.0) {
  ...
  if (Defined("dt"))
    dt = double(param["dt"]);
  if (Defined("configs"))
    Nconfigs = int(param["config"]);
  if (Defined("atoms"))
    Natoms = int(param["atoms"]);
...
void DlPolyFormat::ReadHeader(std::istream& is) const {
  ...
  if (ftype == "HISTORY") {
    ...
    ost >> a1 >> a2 >> a3 >> a4 >> a5;
    if (a1 != level)
      throw PluginError("dlpoly", "Error the level-file are diferent that the specified.");
  }
}
...
bool DlPolyFormat::ReadCell(std::istream& is, Configuration& con) const {
  ...
  if (ftype == "CONFIG") {
    getline(is, tmp); // lee la linea de titulo
    if (is.eof())
      return false; // no hay mas configuraciones que leer
    getline(is, tmp);
    std::istringstream ost(tmp);
    ost >> fkey >> pbk;
    for (int i = 0; i < 3; ++i) {
    ...
    while (1) {
      ...
      for (int i = 0; i <= fkey; ++i) {
        ...
        if (i == 0)
          this_atom.Position() =
              Vector(x + 0.5 * cv[0].Module(), y + 0.5 * cv[1].Module(), z + 0.5 * cv[2].Module());
        if (i == 1)
          this_atom.Velocity() = Vector(x, y, z);
        if (i == 2)
          this_atom.Acceleration() = Vector(x, y, z);
      }
...
void DlPolyFormat::WriteHeader(std::ostream& os, lpmd::SimulationHistory* sh) const {
  ...
  if (ftype == "HISTORY") {
    if (!Defined("dt"))
      throw PluginError("dlpoly", "HISTORY format needs the \"dt\" parameter");
    char buf[512];
    int frame = 0, atoms = 0;
    if (Defined("configs"))
      frame = Nconfigs;
      ...
      if (sh == NULL)
        throw PluginError(
            "dlpoly",
            "Please set the number of configurations, use the configs=N and atoms=N parameters.");
      frame = (*sh).Size();
    }
    if (Defined("atoms"))
      atoms = Natoms;
    ...
    SafeFormat(buf, "%10d%10d%10d%10d%10d\n", level, pbkey, frame, atoms, records);
    os << buf;
    *(stepcnt) = 0;
  } else
    os << "Generated by LPMD dlpoly plugin\n";
...
void DlPolyFormat::WriteCell(std::ostream& out, Configuration& con) const {
  ...
  char buf[512];
  if (ftype == "CONFIG") {
    SafeFormat(buf, "%10d%10d\n", level, pbkey);
    out << buf;
    for (int i = 0; i < 3; ++i) {
      SafeFormat(buf, "%20.10f%20.10f%20.10f\n", cell[i][0], cell[i][1], cell[i][2]);
      out << buf;
    }
    for (long i = 0; i < atoms.Size(); ++i) {
      const Atom& at = atoms[i];
      SafeFormat(buf, "%-5s%13ld\n", at.Symbol().c_str(), i + 1);
      out << buf;
      Vector v;
      if (level >= 0) {
        v = at.Position();
        for (int q = 0; q < 3; ++q)
          v[q] = v[q] - 0.5 * cell[q].Module();
        SafeFormat(buf, "    %12.10g        %12.10g        %12.10g\n", v[0], v[1], v[2]);
        out << buf;
      }
      if (level >= 1) {
        v = at.Velocity();
    ...
    // 256 es el numero de atomos; 0 es el level; 1 es la peridicidad; 0.001000 es el paso del
    // integrador 0.000000 es el paso de tiempo real (integrador por current timestep).
    (*stepcnt)++;
    SafeFormat(buf, "timestep%10ld%10d%10d%10d%12.6g    %12.6g\n", *(stepcnt), int(con.Atoms().Size()),
            level, pbkey, dt, *(stepcnt)*dt);
    out << buf;
    for (int i = 0; i < 3; ++i) {
      SafeFormat(buf, "%20.10f%20.10f%20.10f\n", cell[i][0], cell[i][1], cell[i][2]);
      out << buf;
