...
// Dai, X. D., Kong, Y., Li, J. H., and Liu, B. X., 2006, J. Phys.: Condens. Matter, 18, 45274542.
// 30
//
...
double FinnisSinclairExt::F(const double& rhoi) const { return -A * sqrt(rhoi); }
...
Vector FinnisSinclairExt::PairForce(const Vector& normrij, const double& r) const {
  if (r <= c) {
    ...
    return Vector(0.0e0, 0.0e0, 0.0e0);
}

Vector FinnisSinclairExt::ManyBodies(const Vector& normrij, const double& rhoi, const double& rhoj,
...
Vector FinnisSinclairExt::UpdateCorrections(const double& rho, const int& N,
