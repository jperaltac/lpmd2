...
void ReplicateModifier::ShowHelp() const {
  ...
  std::cout
      << "      The plugin is used to replicate the sample of atoms in the simulation cell.\n";
  std::cout << "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n";
}
...
void ReplicateModifier::Apply(Simulation& con) {
    ...
    for (int j = 0; j < ny; ++j)
      for (int i = 0; i < nx; ++i) {
        for (long int p = 0; p < base_positions.Size(); ++p) {
          Vector scaled_base(base_positions[p][0] / double(nx), base_positions[p][1] / double(ny),
                             base_positions[p][2] / double(nz));
  ...
  for (long int i = 0; i < atoms.Size(); ++i)
    atoms[i].Velocity() = Vector(0.0, 0.0, 0.0);
  for (long int i = 0; i < atoms.Size(); ++i)
