...
void VariableStep::Initialize(Simulation& sim, Potential& p) {
  assert(&p != 0);
  UseOldConfig(sim);
}
...
void VariableStep::Advance(Simulation& sim, Potential& p) {
  ...
  if (atoms.HaveAny(Tag("fixedvel"))) {
    ...
    for (i = 0; i < atoms.Size(); ++i) {
      at = atoms[i];
      if (atoms.Have(at, Tag("fixedvel")))
        atoms[i].Acceleration() = aczero;
    }
  }
  ...
  if (atoms.HaveAny(Tag("fixedpos"))) {
    ...
    for (i = 0; i < atoms.Size(); ++i) {
      at = atoms[i];
      if (atoms.Have(at, Tag("fixedpos")) && (atoms.GetTag(at, Tag("fixedpos")) == "true"))
        continue;
      else {
