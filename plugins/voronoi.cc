...
void Replicate(OrthogonalCell& unitcell, ParticleSet& unitset, unsigned long nx, unsigned long ny,
               unsigned long nz);
void Rotate(OrthogonalCell& unitcell, ParticleSet& unitset, Vector rotate);
void ReplicateRotate( //
    double num, double grains, OrthogonalCell unitcell, ParticleSet unitset, Vector& cellcenter,
    Vector& CellColor, //
...
void VoronoiGenerator::Generate(lpmd::Configuration& conf) const {
  ...
  std::cout << " -> Creating " << grains
            << " grains. Estimated average grain diameter: " << pow(V / grains, 1.0 / 3.0)
            << " angstroms." << std::endl;
  std::cout << " -> Unitary " << type << " cell replicated " << nx << " times in each axis..."
            << std::endl;
  ...
  for (int i = 0; i < grains; ++i) {
    Vector rotate =
        2 * M_PI * drand48() * e1 + 2 * M_PI * drand48() * e2 + 2 * M_PI * drand48() * e3;
    ReplicateRotate(i, grains, basecell, ps, centers[i], CellColor[i], nx, ny, nz, rotate, atoms,
                    celda);
  }

  std::cout << " -> Replicated unitary cells were rotated and moved into the cell with Skew-Start "
               "method..."
            << std::endl;
  ...
  for (long i = 0; i < atoms.Size(); ++i) {
    ...
    if (pos[0] < 0 || pos[0] > x) {
      atoms.Delete(i);
      kill = true;
    } else if (pos[1] < 0 || pos[1] > y) {
      atoms.Delete(i);
      kill = true;
    } else if (pos[2] < 0 || pos[2] > z) {
      atoms.Delete(i);
      kill = true;
    }
  ...
  //-------------------- 3D ELIMINATION -------------------//
  // PAIRS ELIMINATION: NOW THAT THE CELL IS FULL OF CELLS FILLED WITH ATOMS, WE ELIMINATE THE
  // CLOSEST ATOMS
  ...
  // atoms[i].Position()=atoms[i].Position()+1.5*(x*e1+y*e2+z*e3);
...
void SkewStart(int n, double x, double y, double z, Vector* centers) {
  ...
  dz = l / double(n);
  for (long i = 0; i < n; ++i) {
    atomos[i].Position() = celda.FittedInside(
        celda.Cartesian(Vector(dx * double(i) + 0.5, dy * double(i) + 0.5, dz * double(i) + 0.5)));
...
void RandomCenters(int n, double x, double y, double z, Vector* centers) {
  ...
  for (long i = 0; i < n; ++i) {
    atomos[i].Position() =
        celda.FittedInside(celda.Cartesian(Vector(drand48() * x, drand48() * y, drand48() * z)));
    centers[i] = atomos[i].Position();
  }
}
...
void Rotate(OrthogonalCell& unitcell, ParticleSet& unitset, Vector rotate) {
  ...
  for (long int i = 0; i < 3; ++i) {
    ...
    Vector newvec;
    for (int j = 0; j < 3; j++)
      newvec[j] = v0 * rotmat[j][0] + v1 * rotmat[j][1] + v2 * rotmat[j][2];
    unitcell[i] = newvec;
  }
  ...
  for (long int i = 0; i < N; ++i) {
    Vector newvec;
    double v0 = unitset[i].Position()[0], v1 = unitset[i].Position()[1],
           v2 = unitset[i].Position()[2];
    for (int j = 0; j < 3; j++)
      newvec[j] = v0 * rotmat[j][0] + v1 * rotmat[j][1] + v2 * rotmat[j][2];
...
void ReplicateRotate( //
  ...
  //-------------------- 1ST ELIMINATION -------------------//
  // OUTSIDE ELIMINATION: Eliminate the atoms out of the cell
  std::cout << " -> Eliminating atoms out of the cell of the grain number " << num + 1 << "..."
            << std::endl;
  double x = celda[0].Module();
  double y = celda[1].Module();
  double z = celda[2].Module();
