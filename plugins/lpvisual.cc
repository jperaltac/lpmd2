...
LPVisual::LPVisual(std::string args)
    : Plugin("lpvisual", "2.0"), active(false), dispman(0), shm(0), shmsize(0) {
...
LPVisual::~LPVisual() {
  if (shm != 0) {
    ...
    for (int q = 0; q < 3; ++q)
      munmap((void*)(sseg->color[q]), clrsize);
    for (int q = 0; q < 50; ++q)
      munmap((void*)(sseg->stname[q]), stsize);
    for (int q = 0; q < 50; ++q)
      munmap((void*)(sseg->ptname[q]), ptsize);
    for (int q = 0; q < 50; ++q)
      munmap((void*)(sseg->data[q]), datasize);
    for (int q = 0; q < 1; ++q)
      munmap((void*)(sseg->bg), bgsize);
    for (int q = 0; q < 1; ++q)
      munmap((void*)(sseg->gbg), gbgsize);
  }
  munmap(shm, shmsize);
...
void LPVisual::SpawnWindowThread(const lpmd::Simulation& sim) {
  ...
  if (shm == MAP_FAILED)
    throw PluginError("lpvisual", "Shared memory failed.");
  //
  ...
  sseg->height4 = 400;
  sseg->xrange[0] = 0;
  sseg->xrange[1] = 1000, sseg->xrange[2] = 1;
  sseg->yrange[0] = -5;
  sseg->yrange[1] = 10, sseg->yrange[2] = 1;
  sseg->camerapos[0] = M_PI;
  sseg->camerapos[1] = M_PI, sseg->camerapos[2] = M_PI;
  sseg->cameraobj[0] = M_PI;
  sseg->cameraobj[1] = M_PI, sseg->cameraobj[2] = M_PI;
  sseg->cameraup[0] = M_PI;
  sseg->cameraup[1] = M_PI, sseg->cameraup[2] = M_PI;
  sseg->markedatom = -1;
  ...
  // ESTADISTICAS: Reserva memoria para los 50 punteros que almacenarán los nombres de todas las
  // estadisticas
  stsize = 100;
  for (int q = 0; q < 50; ++q) {
  ...
  // GRAFICOS: Reserva memoria para los 50 punteros que almacenarán los nombres de las propiedades
  // que se graficaran
  ptsize = 100;
  for (int q = 0; q < 50; ++q) {
  ...
  for (long int i = 0; i < (sseg->natoms); ++i) {
    if (ColorHandler::HaveColor(atoms[i]))
      for (int j = 0; j < 3; ++j)
        sseg->color[j][i] = ColorHandler::ColorOfAtom(atoms[i])[j];
    else
      for (int j = 0; j < 3; ++j)
        sseg->color[j][i] = ColorHandler::DefaultColor(atoms[i])[j];
  }
  ...
  if (pid == 0) // Este es el hilo que controla la ventana grafica
  {
    ...
    AssignParameter("windowpid", ToString<int>(pid)); // Este es el hilo original
}
...
void LPVisual::Apply(const lpmd::Simulation& sim) {
  ...
  if (Defined("windowpid")) {
    SharedSegment* sseg = (SharedSegment*)(shm);
    if ((!sseg->liveWIN1) && (applied_once))
      throw MissingComponent("lpvisual window");
    else if (!sseg->liveWIN1)
      return;
    ...
    for (long int i = 0; i < atoms.Size(); ++i) {
      for (int q = 0; q < 3; ++q)
        coordbuffer[i * 3 + q] = atoms[i].Position()[q];
    }
