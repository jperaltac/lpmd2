...
LCBinary::~LCBinary() {
  if (nwin + nfail > 0.0)
    DebugStream() << "-> lcbinary efficiency: " << 100.0 * nwin / (nwin + nfail) << "%\n";
  delete[] atomlist;
  delete[] subcell;
}
...
void LCBinary::BuildNeighborList(Configuration& conf, long i, NeighborList& nlist, bool full,
                                 double rcut) {
  BasicParticleSet& atoms = conf.Atoms();
  BasicCell& cell = conf.Cell();
