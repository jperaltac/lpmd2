...
void TempProfile::SetParameter(std::string name) {
  ...
  if (name == "axis") {
    ...
    if (range[tmp][0] >= range[tmp][1] && smin != "all" && smin != "ALL")
      ShowWarning("tempprofile", "min and max values are not consistent.");
  } else
...
void TempProfile::Evaluate(Configuration& con, Potential& pot) {
  ...
  if (bins == 0)
    throw PluginError("tempprofile", "Error in calculation: Wrong value for \"bins\".");
  ...
  if (fabs(range[0][0] - range[0][1]) < 1E-5)
    throw PluginError("tempprofile", "Error in cell range in axis X.");
  if (fabs(range[1][0] - range[1][1]) < 1E-5)
    throw PluginError("tempprofile", "Error in cell range in axis Y.");
  if (fabs(range[2][0] - range[2][1]) < 1E-5)
    throw PluginError("tempprofile", "Error in cell range in axis Z.");
  ...
  if (axis == 0) {
    ...
    throw PluginError("tempprofile", "Error in axis setting to set 'dr'!.");
  }
  ...
  for (int i = 0; i < elements.Size(); ++i) {
          ...
          if (y >= range[1][0] && y <= range[1][1]) {
            if (z >= range[2][0] && z <= range[2][1]) {
              ...
              if (axis == 0)
                ...
                ShowWarning("plugin tempprofile",
                            "Bad calculation of tempprofile, check your 'axis' option.");
              int ir = (long)floor(pp / dr);
              if ((ir >= 0) && (ir < bins)) {
                temp[ir][s] += 0.5 * m * velocity.SquareModule() *
                               kin2ev; // Solo son los aportes a la energia cinetica.
                nab[ir]++;
              }
            }
          }
    ...
    // Reasigna el verdadero valor de tempratura, segun la cantidad de atomos de la especie
    // considerada en cada bins (nab).
