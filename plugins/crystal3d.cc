    ...
    for (long k = 0; k < nz; ++k)
      for (long j = 0; j < ny; ++j)
        for (long i = 0; i < nx; ++i) {
          ...
          atoms[cc++].Position() =
              cell.Cartesian(Vector(double(i) * ax, double(j) * ay, double(k) * az));
          if (create_atoms)
            atoms.Append(Atom(spc));
          atoms[cc++].Position() = cell.Cartesian(
      ...
      for (long j = 0; j < ny; ++j)
        for (long i = 0; i < nx; ++i) {
          if (create_atoms)
            atoms.Append(Atom(spc));
          atoms[cc++].Position() = cell.Cartesian(
              Vector((double(i) + 0.51) * ax, (double(j) + 0.5) * ay, (double(k) + 0.5) * az));
          if (create_atoms)
            atoms.Append(Atom(spc));
          atoms[cc++].Position() = cell.Cartesian(
              Vector((double(i) + 0.51) * ax, double(j) * ay, (double(k) + 1.0) * az));
          if (create_atoms)
            atoms.Append(Atom(spc));
          atoms[cc++].Position() = cell.Cartesian(
              Vector((double(i) + 1.01) * ax, double(j) * ay, (double(k) + 0.5) * az));
          if (create_atoms)
            atoms.Append(Atom(spc));
          atoms[cc++].Position() = cell.Cartesian(
              Vector((double(i) + 1.01) * ax, (double(j) + 0.5) * ay, (double(k) + 1.0) * az));
        }
  } else if (type == "hcp") {
    for (long k = 0; k < nz; ++k)
      for (long j = 0; j < ny; ++j)
        for (long i = 0; i < nx; ++i) {
          if (create_atoms)
            atoms.Append(Atom(spc));
          atoms[cc++].Position() =
              cell.Cartesian(Vector((double(i) + (2.0 / 3.0)) * ax, (double(j) + (1.0 / 3.0)) * ay,
                                    (double(k) + (3.0 / 4.0)) * az));
          if (create_atoms)
            atoms.Append(Atom(spc));
          atoms[cc++].Position() =
              cell.Cartesian(Vector((double(i) + (1.0 / 3.0)) * ax, (double(j) + (2.0 / 3.0)) * ay,
                                    (double(k) + (1.0 / 4.0)) * az));
        }
  } else if (type == "sc") {
    for (long k = 0; k < nz; ++k)
