...
LPMDFormat::LPMDFormat(std::string args) : Plugin("lpmd", "2.0") {
  ...
  if (strcmp(q.c_str(), "lpmd") == 0) {
    type = "lpmd";
  } else if (strcmp(q.c_str(), ".zlp") == 0) {
    type = "zlp";
  }
...
LPMDFormat::~LPMDFormat() {
  if (type == "zlp") {
    if ((*lastop) == ZLP_WRITE)
      deflate((z_stream*)(zstr), Z_FINISH);
    if ((*lastop) == ZLP_READ)
      inflateEnd((z_stream*)(zstr));
    if ((*lastop) == ZLP_WRITE)
      deflateEnd((z_stream*)(zstr));
    delete lastop;
    delete[] inbuf;
    delete[] outbuf;
...
void LPMDFormat::ReadHeader(std::istream& is) const {
  ...
  if (first[0] == 'Z') {
    ...
    throw PluginError("lpmd", "Wrong header (not an LPMD/ZLP file)");
  ...
  if (lspl.Size() > 2) {
    if (lspl[2] == "Z")
      ...
      throw PluginError("lpmd", "Wrong compression flag (corrupted LPMD/ZLP file?)");
  }

  (*linecounter) = 1;

  if (version == "1.0")
    ReadHeaderLPMDOne(is);
  else if (version == "2.0") {
    ...
    if (words[0] != "HDR")
      throw PluginError("lpmd",
                        "File " + readfile + " doesn't seem to be in LPMD 2.0 format (wrong HDR)");
    for (long int i = 0; i < words.Size(); ++i)
      hdr.Append(std::string(words[i]));
  } else
    throw PluginError("lpmd",
                      "Format version " + version + " in file " + readfile + " is not supported.");
  if (type == "zlp")
    InitDecompression();
}
...
void LPMDFormat::ReadHeaderLPMDOne(std::istream& is) const {
  ...
  if (words.Size() == 4) {
    ...
    hdr.Append(std::string("X"));
    hdr.Append(std::string("Y"));
    hdr.Append(std::string("Z"));
    level = 0;
    ...
    hdr.Append(std::string("Y"));
    hdr.Append(std::string("Z"));
    hdr.Append(std::string("VX"));
    hdr.Append(std::string("VY"));
    hdr.Append(std::string("VZ"));
    level = 1;
    ...
    hdr.Append(std::string("VY"));
    hdr.Append(std::string("VZ"));
    hdr.Append(std::string("FX"));
    hdr.Append(std::string("FY"));
    hdr.Append(std::string("FZ"));
    level = 2;
  } else
...
void LPMDFormat::ReadHeaderZLPOne(std::istream& is) const {
  ...
  is.read(&hdr[1], 9);
  if ((hdr[2] != 'L') || (hdr[4] != 'P'))
    throw PluginError("zlp", "Wrong header");
  if ((hdr[1] != '4') || (hdr[3] != '2'))
    throw PluginError("zlp", "Wrong header");
...
void LPMDFormat::InitDecompression() const {
  *lastop = ZLP_READ;
  ...
  if (inflateInit(&stream) != Z_OK)
    throw PluginError("zlp", "Decompression failed");
}
...
bool LPMDFormat::ReadCell(std::istream& is, Configuration& con) const {
  ...
  if (words.Size() == 9) {
    ...
    if ((*this)["replacecell"] == "true") {
      Cell tmp2(atof(words[0].c_str()), atof(words[1].c_str()), atof(words[2].c_str()),
                atof(words[3].c_str()), atof(words[4].c_str()), atof(words[5].c_str()));
      for (int q = 0; q < 3; ++q)
        cell[q] = tmp2[q];
    }
  } else
    throw PluginError("lpmd", "Error ocurred when reading the base vectors, file \"" + readfile +
    ...
    if (words.Size() == 0) {
      ...
      for (long int k = 1; k < hdr.Size(); ++k) {
        if (hdr[k] == "SYM")
          sym = words[k - 1];
        if (hdr[k] == "X")
          X = atof(words[k - 1].c_str());
        if (hdr[k] == "Y")
          Y = atof(words[k - 1].c_str());
        if (hdr[k] == "Z")
          Z = atof(words[k - 1].c_str());
        if (hdr[k] == "VX")
          VX = atof(words[k - 1].c_str());
        if (hdr[k] == "VY")
          VY = atof(words[k - 1].c_str());
        if (hdr[k] == "VZ")
          VZ = atof(words[k - 1].c_str());
        if (hdr[k] == "AX")
          AX = atof(words[k - 1].c_str());
        if (hdr[k] == "AY")
          AY = atof(words[k - 1].c_str());
        if (hdr[k] == "AZ")
          AZ = atof(words[k - 1].c_str());
        if (hdr[k] == "RGB" || hdr[k] == "rgb") {
          color = Vector(words[k - 1].c_str());
          color_active = true;
        }
        if (hdr[k] == "C") {
          color = ColorFromScalar(atof(words[k - 1].c_str()));
          color_active = true;
        }
      }
      ...
      if (color_active)
        ColorHandler::ColorOfAtom(realatom) = color;
    } else
      throw PluginError("lpmd", "An unidentified line was found in the file \"" + readfile +
                                    "\", line " + ToString<int>(*linecounter));
    atomcount++;
  }
  delete istr;
...
void LPMDFormat::WriteHeader(std::ostream& os, SimulationHistory* sh) const {
  ...
  if (type == "zlp") {
    ...
    *lastop = ZLP_WRITE;
    ...
    stream.zfree = Z_NULL;
    stream.opaque = Z_NULL;
    if (deflateInit(&stream, complev) != Z_OK)
      throw PluginError("zlp", "Compression failed");
    os << "Z\n";
  } else
    os << "L\n";
  os << "HDR ";
...
void LPMDFormat::WriteCell(std::ostream& out, Configuration& con) const {
    ...
    if (extra.Size() >= 1) {
      for (long int j = 0; j < extra.Size(); ++j) {
        if ((extra[j] == "RGB") || (extra[j] == "rgb")) {
          lpmd::Vector tmp =
              (ColorHandler::HaveColor(part[i]) ? ColorHandler::ColorOfAtom(part[i])
                                                : ColorHandler::DefaultColor(part[i]));
          obufstr << " ";
          FormattedWrite(obufstr, tmp);
        } else if ((extra[j] == "TYPE") || (extra[j] == "type")) {
          obufstr << "          "
                  << "ATOMTYPE";
        }
      }
  ...
  if (type == "lpmd") {
    ...
    throw PluginError("lpmd", "Not defined the correct type file to write header.");
  delete istr;
  delete ostr;
}
