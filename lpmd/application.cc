...
Application::Application(const std::string& appname, const std::string& cmd, UtilityControl& uc)
    : innercontrol(uc), name(appname), cmdname(cmd) {
  simulation = 0;
  inputfile_stream = 0;
  indexbuffer = 0;
  replay = 0;
...
int Application::Run() {
  ...
  SetPotentials();
  if (!innercontrol.Defined("restore-file"))
    ApplyPrepares(); // do not run if restoring simulation
  if (name == "LPMD") {
    if (innercontrol["filter-at-end"] == "false")
      ApplyFilters();
    else {
      RunModifiers();
      ApplyFilters();
  ...
  if (innercontrol.Defined("cellmanager-module")) {
    simulation->SetCellManager(
        CastModule<CellManager>(pluginmanager[innercontrol["cellmanager-module"]]));
  }
  //
  OpenPropertyStreams();
  OpenOutputStreams();
...
void Application::ProcessControl(int argc, const char* argv[], const std::string& use_hint) {
  ...
  if (quick.Defined("help"))
    ShowHelp();
  else if (quick.Defined("test-plugin"))
    AutoTestPlugin(quick["test-plugin-name"]);
  else {
    if (quick.Arguments().Size() == 1) {
      ...
      if (pluginmanager.IsLoaded("help_plugin"))
        ShowPluginHelp();
      else if (!innercontrol.Defined("cell-type"))
        ShowHelp();
...
void Application::ConstructCell() {
  ...
  if (celltype == "automatic") {
    ...
    for (int q = 0; q < 3; ++q)
      cell[q] = length * scale * identity[q];
    ...
    throw RuntimeError("The cell type must be \'cubic\', \'crystal\' or \'vector\' (or you must "
                       "use the replacecell flag, -r)");
}
...
void Application::ConstructSimulation() {
  ...
  for (int q = 0; q < 3; ++q)
    simulation->Cell()[q] = temporary_cell[q];
}

void Application::FillAtoms() {
  Module& inputmodule = pluginmanager["input1"];
  if (bool(innercontrol["replacecell"]))
    inputmodule["replacecell"] = "true";
  else
    inputmodule["replacecell"] = "false";
  ...
  if (bool(innercontrol["optimize-simulation"]))
    OptimizeSimulationAtStart();
  else
    runtime_context.session().DebugStream() << "-> NOT optimizing simulation, by user request\n";
...
void Application::FillAtomsFromCellReader() {
  ...
  if (pluginmanager.HasType<CellReader>("input1")) {
    ...
    runtime_context.session().DebugStream()
        << "-> Reading input file: " << inputmodule["file"] << '\n';
...
void Application::UpdateAtomicIndices() {
  ...
  for (long int i = 0; i < atoms.Size(); ++i)
    atoms.SetTag(atoms[i], indextag, const_cast<const std::string&>(indexbuffer[i]));
}

void Application::AdjustAtomProperties() {
  if (simulation == 0)
    throw InvalidOperation("Adjusting atomic properties on an uninitialized Simulation");
  BasicParticleSet& atoms = simulation->Atoms();
  ...
  if (innercontrol.Defined("showinitialconfig")) {
    for (long int i = 0; i < atoms.Size(); ++i) {
      std::cout << i << ": " << atoms[i].Symbol() << " Mass = " << atoms[i].Mass()
...
void Application::ApplyPrepares() {
  ...
  for (int p = 0; p < prepares.Size(); ++p) {
    ...
    sm.Apply(*simulation);
    if (simulation->Cell() != original_cell)
      simulation->RescalePositions(original_cell);
  }
  if (innercontrol.Defined("cellmanager-module"))
    CastModule<CellManager>(pluginmanager[innercontrol["cellmanager-module"]])
        .UpdateCell(*simulation);
}

void Application::ApplyFilters() {
  if (name == "LPMD")
    PrintBanner("FILTER MODULES");
  ...
  for (int p = 0; p < filters.Size(); ++p) {
    ...
    bool inverse = false;
    if (pluginmanager[id].Defined("inverse") && (bool(pluginmanager[id]["inverse"])))
      inverse = true;
    sfilt.inverted = inverse;
    if ((rawmodule.Defined("filterby")) && (rawmodule["filterby"] != "none")) {
      runtime_context.session().DebugStream()
          << "-> Applying implicit filter on " << rawmodule.Name() << '\n';
      ...
      Selector<BasicParticleSet>& selector =
          (CastModule<SystemFilter>(filtering_plugin)).CreateSelector();
      bool inverse2 = false;
      if (filtering_plugin.Defined("inverse") && (bool(filtering_plugin["inverse"])))
        inverse2 = true;
      //
      ...
      if (!inverted)
        filtered = new ParticleSet(selector2.SelectFrom(atoms));
      else
        filtered = new ParticleSet(selector2.InverseSelectFrom(atoms));
      simulation->RemoveAtomMask();
      simulation->Atoms().Clear();
      for (long int i = 0; i < filtered->Size(); ++i)
        simulation->Atoms().Append((*filtered)[i]);
      delete filtered;
      //
      //
      //
...
void Application::SetPotentials() {
  if (simulation == 0)
    throw InvalidOperation("Setting potentials on an uninitialized Simulation");
  CombinedPotential& potentials = simulation->Potentials();
  Array<Parameter> pkeys = innercontrol.Potentials().Parameters();
  if (name == "LPMD")
    PrintBanner("INTERATOMIC POTENTIALS");
  for (int p = 0; p < pkeys.Size(); ++p) {
    ...
    if ((species_a != "all") && (species_b != "all")) {
      ...
      pot.SetValidSpecies(-1, -1); // wildcard meaning "all species"
    potentials.Append(pot);
    if (name == "LPMD")
      plugin.Show(std::cout);
  }
}
...
template <typename T>
void ApplySteppers(PluginManager& pluginmanager, UtilityControl& control, Simulation& simulation,
  ...
  for (int p = 0; p < modules.Size(); ++p) {
    try {
      ...
      if (mod.IsActiveInStep(currentstep)) {
        if ((rawmodule.Defined("filterby")) && (rawmodule["filterby"] != "none")) {
          simulation.Context().session().DebugStream()
              << "-> Applying implicit filter on " << modules[p] << '\n';
          ...
          Cell original_cell(simulation.Cell());
          if ((mod.end == -1) && control.Defined("steps-number"))
            mod.end = int(control["steps-number"]);
          try {
      ...
      continue;
    } // A stepper was unloaded 'on the fly', skip it
  }
}

void Application::RunModifiers() {
  ApplySteppers<SystemModifier>(pluginmanager, innercontrol, *simulation, "apply");
}

void Application::RunVisualizers() {
  ApplySteppers<Visualizer>(pluginmanager, innercontrol, *simulation, "visualize");
}
...
void Application::ComputeProperties() {
  ...
  for (int p = 0; p < properties.Size(); ++p) {
    ...
    if (prop.IsActiveInStep(currentstep)) {
      if ((rawmodule.Defined("filterby")) && (rawmodule["filterby"] != "none")) {
        runtime_context.session().DebugStream()
            << "-> Applying implicit filter on " << properties[p] << '\n';
        runtime_context.session().DebugStream() << "-> Filtered plugin details:\n";
        rawmodule.Show(runtime_context.session().DebugStream());
        Module& filtering_plugin = pluginmanager[rawmodule["filterby"]];
        runtime_context.session().DebugStream() << "-> Filtering plugin details:\n";
      ...
      if (bool(pluginmanager[properties[p]]["average"]))
        value.AddToAverage();
      else
        value.OutputTo(*(propertystream[p]));
    }
  }
}
...
void Application::OpenPropertyStreams() {
  if (name == "LPMD")
    PrintBanner("INSTANTANEOUS PROPERTIES");
