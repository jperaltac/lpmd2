...
std::string QuickModeParser::FormattedAsControlFile() const {
  ...
  if (Defined("pluginhelp-name"))
    control += "use " + (*this)["pluginhelp-name"] + " as help_plugin\nenduse\n";
  if (Defined("verbose"))
    control += "set verbose true\n";
  if (Defined("replace-cell"))
  ...
  if (Defined("option-keywordvalue")) {
    ...
    for (int p = 0; p < options.Size(); ++p) {
      ...
      if (pair.Size() != 2)
        throw SyntaxError("Setting options with -O");
  ...
  if (Defined("vector")) {
    ...
    if (comp.Size() != 9)
      throw SyntaxError("Only nine vector components are supported with -V");
    cellstatement = "cell vector ax=" + comp[0] + " ay=" + comp[1] + " az=" + comp[2];
    cellstatement += (" bx=" + comp[3] + " by=" + comp[4] + " bz=" + comp[5] + " cx=" + comp[6] +
                      " cy=" + comp[7] + " cz=" + comp[8]);
  } else {
    if (Defined("lengths")) {
      Array<std::string> box = StringSplit((*this)["lengths-a,b,c"], ',');
      if (box.Size() == 1)
        cellstatement = "cell cubic a=" + box[0];
      else if (box.Size() == 3)
        cellstatement = "cell crystal a=" + box[0] + " b=" + box[1] + " c=" + box[2];
      else
        throw SyntaxError("Only one or three arguments are supported with -L");
    }
    if (Defined("angles")) {
      Array<std::string> angles = StringSplit((*this)["angles-alpha,beta,gamma"], ',');
      if (angles.Size() == 3)
        cellstatement += " alpha=" + angles[0] + " beta=" + angles[1] + " gamma=" + angles[2];
      else
        throw SyntaxError("Only three arguments are supported with -A");
    }
  }
  if (Defined("periodic")) {
  ...
  if (Defined("scale-value"))
    cell_scale = (*this)["scale-value"];
  if (cellstatement != "")
    control += (cellstatement + " scale=" + cell_scale + "\n");
  if (Defined("input-options"))
    control += ("input " + ParseModuleLoading((*this)["input-options"]));
  if (Defined("output-options"))
    control += ("output " + ParseModuleLoading((*this)["output-options"]));
  if (Defined("use-options"))
    control += ParseUseOptions((*this)["use-options"], impl->autouse_statement);
