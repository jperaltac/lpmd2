...
ReplayIntegrator::ReplayIntegrator(Plugin& inpplugin, std::istream& inpstream)
    : inputstream(inpstream) {
  nconf = 1;
  if (!inpplugin.Defined("start"))
    inpplugin["start"] = "0";
  if (!inpplugin.Defined("end"))
    inpplugin["end"] = "-1";
  if (!inpplugin.Defined("each"))
    inpplugin["each"] = "1";
  mystepper.start = int(inpplugin["start"]);
  mystepper.end = int(inpplugin["end"]);
  mystepper.each = int(inpplugin["each"]);
  try {
    cellreader = dynamic_cast<CellReader*>(&inpplugin);
  ...
  if (cellreader != 0) {
    ...
    if (mystepper.IsActiveInStep(nconf)) {
      ...
      try {
        ...
        status = cellreader->ReadCell(inputstream, sim);
        sim.Context().session().DebugStream()
            << "-> Read (could not skip) configuration " << nconf << "\n";
      }
      nconf++;
...
void ReplayIntegrator::PreRead(Simulation& simulation) {
  ...
  simulation.Context().session().DebugStream()
      << "-> Read " << history.Size() << " configurations.\n";
}
