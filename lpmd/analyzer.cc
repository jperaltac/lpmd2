...
Analyzer::Analyzer(int argc, const char* argv[])
    : Application("LPMD Analyzer", "lpmd-analyzer", control), control(pluginmanager) {
  std::cerr << "\nLPMD Analyzer, version " << VERSION << "\n\n";
  ProcessControl(argc, argv, "property");
}

Analyzer::~Analyzer() { std::cerr << "\n-> Done.\n"; }
