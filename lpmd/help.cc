...
void Application::ShowHelp() {
  ...
  std::cerr << '\n';
  std::cerr << "Using liblpmd version " << runtime_context.session()["libraryversion"] << std::endl
            << std::endl;
  std::cerr << "Usage: " << cmdname
            << " [--verbose | -v ] [--lengths | -L <a,b,c>] [--angles | -A <alpha,beta,gamma>]";
  std::cerr << " [--vector | -V <ax,ay,az,bx,by,bz,cx,cy,cz>] [--scale | -S <value>]";
  std::cerr << " [--option | -O <option=value,option=value,...>] [--input | -i "
               "plugin:opt1,opt2,...] [--output | -o plugin:opt1,opt2,...]";
  std::cerr << " [--use | -u plugin:opt1,opt2,...] [--cellmanager | -c plugin:opt1,opt2,...] "
