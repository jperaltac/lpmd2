...
void UtilityControl::Read(const std::string& filename, const ParamList& options) {
  ...
  if (bool((*this)["replacecell"]))
    (*this)["cell-type"] = "automatic";
}

void UtilityControl::Read(std::istream& istr, const ParamList& options,
...
int UtilityControl::OnRegularStatement(const std::string& name, const std::string& keywords) {
  ...
  ParamList& params = (*this);
  if (name == "cell") {
    if ((!params.Defined("cell-b")) && (!params.Defined("cell-ay")))
      params["cell-type"] = "cubic";
    else if (params.Defined("cell-b"))
      params["cell-type"] = "crystal";
    else if (params.Defined("cell-ax"))
      params["cell-type"] = "vector";
  } else if (name == "set") {
    // FIXME: we catch special key names to translate them...
    if (params["set-option"] == "filter-end")
      params["set-option"] = "filter-at-end";
    //
    params[params["set-option"]] = params["set-value"];
    params.Remove("set-option");
    params.Remove("set-value");
  } else if (name == "potential") {
    impl->potentials[params["potential-a"] + "-" + params["potential-b"]] =
        params["potential-module"];
  } else if (name == "bond") {
    impl->bonds[params["bond-a"] + "-" + params["bond-b"]] = params["bond-length"];
  } else if (name == "plugindir")
    impl->pluginpath.Append(params["plugindir-dir"]);
  else if (name == "mass")
    ...
    module_info.id = name;
    module_info.args =
        ("properties " + params[name + "-properties"] + " output " + params[name + "-output"]);
...
int UtilityControl::OnNonRegularStatement(const std::string& name,
                                          const std::string& full_statement) {
  Map& params = (*this);
  ...
  if (((name == "input") || (name == "prepare")) || (name == "output")) {
    ...
    if (over_pos > 0) {
      ...
      for (int i = 0; i < over_pos; ++i)
        main_statement += (st_words[i] + " ");
      main_statement += ("filterby=implicitfilter" + ToString(impl->implfilter_count));
      std::string impl_filter =
          ("implicitfilter id=implicitfilter" + ToString(impl->implfilter_count) + " module=");
      for (int i = over_pos + 1; i < st_words.Size(); ++i)
        impl_filter += (st_words[i] + " ");
      ...
      impl->modulecounter[name]++;
      std::string tmp = ParseCommandArguments(params, name, "module ");
      std::string validkeywords =
          (impl->pluginmanager)->GetPluginKeywords(params[name + "-module"]);
      std::string args = ParseCommandArguments(params, name, validkeywords);
      params[name + "-modules"] += (params[name + "-module"] + " ");
      module_info.name = params[name + "-module"];
      module_info.id = name + ToString(impl->modulecounter[name]);
      ...
      for (int q = 0; q < impl->plugins.Size(); ++q) {
        if (impl->plugins[q].id == params[name + "-module"]) {
          ...
          if (name == "property") {
            try {
              InstantProperty& ip =
                  CastModule<InstantProperty>((*(impl->pluginmanager))[params[name + "-module"]]);
              assert(&ip != 0);
            } catch (Error& e) {
              try {
                TemporalProperty& tp = CastModule<TemporalProperty>(
                    (*(impl->pluginmanager))[params[name + "-module"]]);
                assert(&tp != 0);
              } catch (Error& e) {
                throw InvalidOperation("Using the " + params[name + "-module"] +
                                       " plugin with the \'property\' statement");
              }
            }
          } else if (name == "apply") {
            try {
              SystemModifier& sm =
                  CastModule<SystemModifier>((*(impl->pluginmanager))[params[name + "-module"]]);
              assert(&sm != 0);
            } catch (Error& e) {
              throw InvalidOperation("Using the " + params[name + "-module"] +
                                     " plugin with the \'apply\' statement");
            }
          } else if (name == "visualize") {
            try {
              Visualizer& vi =
                  CastModule<Visualizer>((*(impl->pluginmanager))[params[name + "-module"]]);
              assert(&vi != 0);
            } catch (Error& e) {
              throw InvalidOperation("Using the " + params[name + "-module"] +
                                     " plugin with the \'visualize\' statement");
            }
          }
          //
...
int UtilityControl::OnBlock(const std::string& name, const std::string& full_statement) {
  if (name == "use") {
    ...
    for (int p = pstart; p < tokens.Size(); ++p)
      module_info.args += (tokens[p] + " ");
