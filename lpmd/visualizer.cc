...
void Visualizer::IterateSequential() {
  while (true) {
    if (bool(control["verbose"]))
      simulation->ShowInfo(std::cout);
    try {
      simulation->GetCellManager().UpdateCell(*simulation);
    } catch (Error& e) {
    } // FIXME: Chequear de mejor manera la presencia del cellmanager
    UpdateAtomicIndices();
    ...
    if (control.Defined("delay"))
      usleep(long(double(control["delay"]) * 1000000.0));
    RunVisualizers();
...
Visualizer::Visualizer(int argc, const char* argv[])
    : Application("LPMD Visualizer", "lpmd-visualizer", control), control(pluginmanager) {
  std::cerr << "\nLPMD Visualizer, version " << VERSION << "\n\n";
  ProcessControl(argc, argv, "visualize");
}

Visualizer::~Visualizer() { std::cerr << "-> Done.\n"; }
